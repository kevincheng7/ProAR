...
class Protein:
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) == 1 and isinstance(args[0], System):
            ...
            X, C, S = args
            assert isinstance(C, torch.Tensor), f"arg[1] must be a chain (C) torch.Tensor, but get {type(C)}"
            assert isinstance(S, torch.Tensor), f"arg[2] must be a sequence (S) torch.Tensor, but get {type(S)}"
            ...
            if args[0].lower().startswith("s3:"):
                raise NotImplementedError("download of cifs or pdbs from s3 not supported.")
            ...
            if args[0].endswith(".cif"):
                ...
                if VALID_PDBID:
                    ...
                    raise NotImplementedError("Could Not Identify a valid input Type. See docstring for" " details.")
        else:
            raise NotImplementedError("Inputs must either be a 3-tuple of XCS tensors, or a single string")
    ...
    @classmethod
    def from_XCS_trajectory(cls, X_traj: List[torch.Tensor], C: torch.Tensor, S: torch.Tensor) -> Protein:
        """
    ...
    @classmethod
    def from_CIF(cls, input_file: str, canonicalize: bool = True, device: str = "cpu") -> Protein:
        """
    ...
    @classmethod
    def from_PDBID(cls, pdb_id: str, canonicalize: bool = True, device: str = "cpu") -> Protein:
        """
    ...
    @classmethod
    def from_sequence(cls, chains: Union[List[str], str], device: str = "cpu") -> Protein:
        """
    ...
    def to_XCS_trajectory(
        ...
        for i in range(self.sys.num_models()):
            ...
            if i == 0:
                ...
                X_frame.flatten(0, 2)[:] = torch.from_numpy(self.sys._locations["coor"][loc_indices, 0:3])
    ...
    def to(self, file_path: str, force: bool = False) -> None:
        ...
        if file_path.lower().endswith(".pdb"):
            ...
            raise NotImplementedError("file path must end with either *.cif or *.pdb for export.")
    ...
    def __str__(self):
        ...
        for chain in self.sys.chains():
            if chain.sequence is not None:
                protein_string += f"> Chain {chain.cid} ({len(chain.sequence())} residues)\n"
                protein_string += "".join([polyseq.to_single(s) for s in chain.sequence()])
                protein_string += "\n\n"
