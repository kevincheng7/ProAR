...
class SystemAssemblyInfo:
    ...
    def rename_chain(self, old_cid: str, new_cid: str):
        ...
        for ass_id, assembly in self.assemblies.items():
            for ins in assembly["instructions"]:
                ins["chains"] = [new_cid if cid == old_cid else cid for cid in ins["chains"]]
...
class StringList:
    ...
    def insert(self, i: int, new_string: str):
        if i < len(self):
            ...
            raise Exception(f"cannot insert in position {i} for stringList of length {len(self)}")
...
class HierarchicList:
    ...
    def copy(self):
        new_list = HierarchicList(self._properties, self._parent_list, self._num_children)
    ...
    def reindex(self):
        if self._num_children is not None:
            self._child_offset = ArrayList(1, dtype=int, length=len(self._num_children), val=0)
            for i in range(1, len(self)):
                self._child_offset[i] = self._child_offset[i - 1] + self._num_children[i - 1]
    ...
    def append(self, properties):
        ...
        if self._child_offset is not None:
            self._child_offset.append(self._child_offset[-1:].sum() + self._num_children[-1:].sum())
        if self._num_children is not None:
...
class System:
            ...
            for residue in chain.residues():
                ...
                for atom in residue.atoms():
                    ...
                    for loc in atom.locations():
                        ...
                        print(f"\t\t\tlocation {loc._ix}, {loc}: has children? {has_children}")
    ...
    def from_XCS(
        ...
        for i, chain_id in enumerate(np.unique(chain_ids)):
            ...
            for chain_ix, (X_i, C_i, S_i) in enumerate(zip(X_chain, C_chain, S_chain)):
                ...
                # Build residue
                residue = chain.add_residue(resname, chain_ix + 1, str(chain_ix + 1), " ")

                if C_i > 0:
                    atom_names = constants.ATOMS_BB

                    if all_atom and resname in constants.AA_GEOMETRY:
                        atom_names = atom_names + constants.AA_GEOMETRY[resname]["atoms"]
        ...
        for ci, chain in enumerate(new_system.chains()):
            ...
            entity = SystemEntity(entity_type, f"chain {chain.cid}", polymer_type, seq, het)
    ...
    def to_XCS(
        ...
        location_indices = np.zeros([num_residues * num_atoms], dtype=int) if get_indices else None
        ...
        for i in range(num_residues):
            ...
            if is_mask:
                ...
                if all_atom:
                    ...
                    atom_names = constants.ATOMS_BB + constants.AA_GEOMETRY[code3]["atoms"]
                    atom_names = {a: i for (i, a) in enumerate(atom_names)}

                X[i, :] = np.nan  # so we can tell whether some atom was previously found
    ...
    def update_with_XCS(self, X, C=None, S=None, alternate_alphabet=None):
        ...
        if not ((X.shape[1] == self.num_residues()) and (X.shape[1] == C.shape[1]) and (X.shape[1] == S.shape[1])):
            raise Exception(
    ...
    def get_residue(self, gti: int):
        ...
        for chain in self.chains():
            ...
            off = off + nr
        raise Exception(f"residue index {gti} out of range for System, which has {self.num_residues()} residues")
    ...
    def get_atom(self, aidx: int):
        ...
        for chain in self.chains():
            ...
            off = off + na
        raise Exception(f"atom index {aidx} out of range for System, which has {self.num_atoms()} atoms")
    ...
    def select(self, expression: str, left_associativity: bool = True):
        ...
        Returns:
            List of AtomView's.
        """
        val, selex_info = self._select(expression, left_associativity=left_associativity)
    ...
    def select_chains(self, expression: str, allow_unstructured=False, left_associativity: bool = True):
    ...
    def _select(
        ...
        if not unstructured:
            val = {i for i in val if not hasattr(_selex_info["all_atoms"][i].atom, "dummy")}
    ...
    def save_selection(
        ...
        if gti is not None:
            if expression is not None:
                warnings.warn(
                    "Expression and gti are both not null, expression will be ignored" " and gti will be used!"
                )
            result = sorted(gti)
        else:
    ...
    def get_selected(self, selname: str = "_default"):
        ...
        if selname not in self._selections:
            raise Exception(f"selection by name '{selname}' does not exist in the System")
    ...
    def _selex_eval(self, _selex_info, op: str, left, right):
        ...
        def _unpack_operands(operands, dests):
            ...
            for i, (operand, dest) in enumerate(zip(operands, dests)):
                if dest is None:
                    ...
                    if not (isinstance(operand, list) and all([isinstance(val, str) for val in operands])):
                        succ = False
                        break
                    unpacked[i] = operands
                elif dest == "float":
                    ...
                    if not (isinstance(operand, list) and all([_is_numeric(val) for val in operands])):
                        succ = False
                        break
                    ...
                    if not (isinstance(operand, list) and all([_is_int(val) for val in operands])):
                        succ = False
                        break
        ...
        def _parse_range(operands: list):
            ...
            if not (isinstance(operands, list) and all([isinstance(opr, str) for opr in operands])):
                return None
        ...
        if op in ("and", "or"):
            ...
            atom_indices = np.asarray([ai.aix for ai in _selex_info["all_atoms"] for xi in ai.atom.locations()])
            X_i = np.asarray([[xi.x, xi.y, xi.z] for ai in _selex_info["all_atoms"] for xi in ai.atom.locations()])
            X_j = np.asarray([[xi.x, xi.y, xi.z] for j in S for xi in _selex_info["all_atoms"][j].atom.locations()])
            ...
            (_, test), succ = _unpack_operands([left, right], [None, "int_range_string"])
            if not succ:
                return None
            for a in _selex_info["all_atoms"]:
    ...
    def _append_residue(self, name: str, num: int, authid: str, icode: str):
        ...
        """
        self._chains.append_child({"name": name, "resnum": num, "authresid": authid, "icode": icode})
    ...
    def _pick_unique_chain_name(self, hint: str, verbose=False):
        goodNames = list("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789")
    ...
    def _read_cif(f, strict=False):
        ...
        is_read = {part: False for part in ["coors", "entities", "sequence", "entity_poly"]}
        category = ""
                ...
                if (cat == "_entry") and (name == "id"):
                    ...
                    for row in tab:
                        ...
                        if ent_id not in system._entities:
                            system._entities[ent_id] = SystemEntity(None, None, row[1], None, None)
                        else:
                            system._entities[ent_id]._polymer_type = row[1]
                    is_read["entity_poly"] = True
                elif cat == "_entity":
                    if is_read["entities"]:
                        raise Exception(f"entities block encountered multiple times: {peeked.line}")
                    ...
                    for row in tab:
                        ent_id = int(row[0]) - 1
                        if ent_id not in system._entities:
                            system._entities[ent_id] = SystemEntity(row[1], row[2], None, None, None)
                        else:
                    ...
                    if is_read["sequence"]:
                        raise Exception("sequence block encountered multiple times")
                    tab = sp.star_read_data(f, ["entity_id", "num", "mon_id", "hetero"], in_loop)
                    (seq, het) = ([], [])
                    ...
                    tab = sp.star_read_data(f, ["assembly_id", "oper_expression", "asym_id_list"], in_loop)
                    for row in tab:
                        ...
                        assembly["instructions"].append({"oper_expression": row[1], "chains": chain_ids})
                    ...
                    for row in tab:
                        system._assembly_info.operations[row[0]] = SystemAssemblyInfo.make_operation(
                            row[1], row[2], row[3:12], row[12:15]
                        )
                elif cat == "_generate_selections":
                    tab = sp.star_read_data(f, ["name", "indices"], in_loop)
                    for row in tab:
                        system._selections[row[0]] = [int(gti.strip()) for gti in row[1].strip().split()]
                    ...
                    if is_read["coors"]:
                        raise Exception("ATOM_SITE block encountered multiple times")
                    ...
                    for i in range(len(tab)):
                        if i == 0:
                        ...
                        elif (tab[i][modelCol] != prev_model) or (tab[i][modelCol] != first_model):
                            ...
                            same = (
                                ...
                                and (
                                    int(sp.star_value(tab[i][seqIdCol], loc.atom.residue.num)) == loc.atom.residue.num
                                )
                            ...
                            coor = [float(tab[i][c]) for c in [xCol, yCol, zCol, occCol, bCol]]
                            system._extra_models[-1]["coor"][aIdx] = coor
                            system._extra_models[-1]["alt"][aIdx] = sp.star_value(tab[i][altIdCol], " ")[0]
                            aIdx = aIdx + 1
                            continue
                        ...
                        if (
                            ...
                            authid = (
                                tab[i][authChainNameCol] if (tab[i][authChainNameCol] != "") else tab[i][chainNameCol]
                            )
                        ...
                        x, y, z, occ, B = [float(tab[i][col]) for col in [xCol, yCol, zCol, occCol, bCol]]
                        alt = sp.star_value(tab[i][altIdCol], " ")[0]
                        if (atom is None) or (residue != prev_residue) or (tab[i][atomNameCol] != atom.name):
                            ai = system._append_atom(tab[i][atomNameCol], (tab[i][groupCol] == "HETATM"))
        ...
        for chain in system.chains():
            ...
            for ri in range(len(entity._seq)):
                ...
                if (cur_res is None) or (cur_res.num > ri + 1):
                    ...
                    _warn_or_error(strict, f"inconsistent numbering in chain {chain.cid}")
                    break
                k = k + 1
    ...
    def _read_pdb(f, strict=False, options=""):
        ...
        for line in f:
            ...
            if line.startswith("ENDMDL"):
                ...
                if model_index:
                    try:
                        ...
                        warnings.warn(f"error when adding model {model_index + 1}: {str(e)}, skipping model...")
                system = System("system")
            ...
            if (chain_id != last_chain_id) or ter:
                ...
                if uniq_chain_ids and cid_used:
                    ...
                    if verbose:
                        warnings.warn(
                            "chain name '" + chain_id + "' was repeated while reading, will rename at the end..."
                        )
            ...
            if (resnum != last_resnum) or (resname != last_resname) or (icodes_as_sep_res and (icode != last_icode)):
                ...
                residue = chain.add_residue(resname, chain.num_residues() + 1, str(resnum), icode[0])
            elif alt != " ":
        ...
        for chain in chains_to_rename:
            ...
            assert len(parts) > 1, "something went wrong when renaming a chain at the end of reading"
    ...
    def _write_cif(self, f):
        ...
        sp.star_loop_header_write(f, "_entity", ["id", "type", "pdbx_description", "pdbx_number_of_molecules"])
        ...
        sp.star_loop_header_write(f, "_entity_poly_seq", ["entity_id", "num", "mon_id", "hetero"])
        for id, entity in self._entities.items():
        ...
        if self.num_assemblies():
            ...
            for assembly_id, assembly in assemblies.items():
                for instruction in assembly["instructions"]:
                    ...
                    f.write(f"{assembly_id} {sp.star_string_escape(instruction['oper_expression'])} {chain_list}\n")
            f.write("#\n")
            ...
            for op_id, op in ops.items():
                f.write(f"{op_id} {sp.star_string_escape(op['type'])} {sp.star_string_escape(op['name'])} ")
                f.write(f"{float(op['matrix'][0][0]):g} {float(op['matrix'][0][1]):g} {float(op['matrix'][0][2]):g} ")
                f.write(f"{float(op['matrix'][1][0]):g} {float(op['matrix'][1][1]):g} {float(op['matrix'][1][2]):g} ")
                f.write(f"{float(op['matrix'][2][0]):g} {float(op['matrix'][2][1]):g} {float(op['matrix'][2][2]):g} ")
                f.write(f"{float(op['vector'][0]):g} {float(op['vector'][1]):g} {float(op['vector'][2]):g}\n")
            f.write("#\n")
        ...
        for model_index in range(self.num_models()):
            ...
            for chain, entity_id in zip(self.chains(), self._chain_entities):
                authchainid = chain.authid if sp.star_value_defined(chain.authid) else chain.cid
                for residue in chain.residues():
                    authresid = residue.authid if sp.star_value_defined(residue.authid) else residue.num
                    for atom in residue.atoms():
                        idx = idx + 1
                        for location in atom.locations():
                            ...
                            f.write(f"{idx + 1} {atom.name} {sp.atom_site_token(location.alt)} ")
                            entity_id_str = f"{entity_id + 1}" if entity_id is not None else "?"
                            f.write(f"{residue.name} {chain.cid} {entity_id_str} {residue.num} ")
                            f.write(f"{sp.atom_site_token(residue.icode)} {coor[0]:g} {coor[1]:g} {coor[2]:g} ")
        ...
        if len(self._selections):
            ...
            for name, indices in self._selections.items():
                f.write(f"{sp.star_string_escape(name)} \"{' '.join([str(i) for i in indices])}\"\n")
            f.write("#\n")
        ...
        if len(self._labels):
            ...
            for category, label_dict in self._labels.items():
                for gti, label in label_dict.items():
                    f.write(f"{sp.star_string_escape(category)} {gti} {sp.star_string_escape(label)}\n")
            f.write("#\n")
    ...
    def _write_pdb(self, f, options=""):
        def _pdb_line(loc: AtomLocationView, ai: int, ri=None, rn=None, an=None):
            ...
            line = "%6s%5d %-4s%c%-4s%.1s%4d%c   %8.3f%8.3f%8.3f%6.2f%6.2f      %.4s" % (
        ...
        if charmm19Format and charmm22Format:
            raise Exception("CHARMM 19 and 22 formatting options cannot be specified together")
    ...
    def canonicalize_protein(
        ...
        def _mod_to_standard_aa_mappings(less_standard: bool, almost_standard: bool, standard: bool):
        ...
        def _to_standard_aa_mappings(less_standard: bool, almost_standard: bool, standard: bool):
            # get the mapping between modifications and their corresponding standard forms
            mapping = _mod_to_standard_aa_mappings(less_standard, almost_standard, standard)
        ...
        for ci, chain in enumerate(self.chains()):
            entity = chain.get_entity()
            if filter_by_entity:
                if (entity is None) or (entity._type != "polymer") or ("polypeptide" not in entity.polymer_time):
            ...
            for residue in reversed(list(chain.residues())):
                ...
                if aa in to_standard:
                    ...
                    if aa != aa_new:
                        ...
                        if ((aa == "HSD") or (aa == "HSE") or (aa == "HSC") or (aa == "HSP")) and (aa_new == "HIS"):
                            pass
                        elif ((aa == "MSE") and (aa_new == "MET")) or ((aa == "SEC") and (aa_new == "CYS")):
                        ...
                        if entity_id is None:
                            ...
                            residues_to_rename[entity_id][ci].append((residue.get_index_in_chain(), aa_new))
                    else:
                        if aa == "ARG":
                            ...
                            if all([a is not None for n, a in A.items()]):
                                dihe1 = System.dihedral(A["CD"], A["NE"], A["CZ"], A["NH1"])
                                dihe2 = System.dihedral(A["CD"], A["NE"], A["CZ"], A["NH2"])
            ...
            if not filter_by_entity and (cleared_residues != 0) and (cleared_residues == chain.num_residues()):
                chains_to_delete.append(chain)
    ...
    @staticmethod
    def angle(a1: AtomLocationView, a2: AtomLocationView, a3: AtomLocationView, radians=False):
    ...
    def protein_backbone_atom_type(atom_name: str, no_hyd=True, by_name=True):
        ...
        if not no_hyd:
            ...
            if atom_name.startswith("1H") or atom_name.startswith("2H") or atom_name.startswith("3H"):
                return array[4]
        return None
    ...
    def guess_entity_and_polymer_type(cls, seq: List):
        ...
        if is_poly:
            ...
            for ptype in polyseq.polymerType:
                if np.mean([polyseq.is_polymer_residue(res, ptype) for res in seq]) > 0.8:
...
class ChainView(BaseView):
    ...
    def get_residue(self, ri: int):
        ...
        if ri < 0 or ri >= self.num_residues():
            raise Exception(f"residue index {ri} out of range for Chain, which has {self.num_residues()} residues")
...
class ResidueView(BaseView):
    ...
    def get_atom(self, ai: int):
        ...
        if ai < 0 or ai >= self.num_atoms():
            raise Exception(f"atom index {ai} out of range for Residue, which has {self.num_atoms()} atoms")
...
class AtomView(BaseView):
    ...
    def get_location(self, li: int = 0):
        ...
        if li < 0 or li >= self.num_locations():
            raise Exception(f"location index {li} out of range for Atom with {self.num_locations()} locations")
    ...
    def add_location(self, x, y, z, occ=1.0, B=0.0, alt=" ", at=None):
        ...
        li = self._siblings.insert_child(self._ix, at, {"coor": [x, y, z, occ, B], "alt": alt})
        return AtomLocationView(li, self)
...
class DummyAtomView(AtomView):
    ...
    def get_location(self, li: int = 0):
        raise Exception("no locations in DUMMY atom")

    def add_location(self, x, y, z, occ, B, alt, at=None):
        raise Exception("can't add no locations to DUMMY atom")
    ...
    @property
    def x(self):
        raise Exception("no coordinates in DUMMY atom")
    ...
    @property
    def occ(self):
        raise Exception("no occupancy in DUMMY atom")

    @property
    def B(self):
        raise Exception("no B-factor in DUMMY atom")

    @property
    def alt(self):
        raise Exception("no alt flag in DUMMY atom")

    @x.setter
    def x(self, val):
        raise Exception("can't set coordinate for DUMMY atom")
    ...
    @occ.setter
    def occ(self, val):
        raise Exception("can't set occupancy for DUMMY atom")

    @B.setter
    def B(self, val):
        raise Exception("can't set B-factor for DUMMY atom")

    @alt.setter
    def alt(self, val):
        raise Exception("can't set alt flag for DUMMY atom")
...
class ExpressionTreeEvaluator:
    ...
    def _traverse_expression_tree(self, E, i=0, eval_all=True, debug=False):
        ...
        def _my_eval(op, left, right, debug=False):
            if debug:
                print(f"\t-> evaluating {operand_str(left)} | {op} | {operand_str(right)}")
        ...
        while i < len(E):
            if all([x is None for x in (left, right, op)]):
                ...
                if E[i] == "(":
                    ...
                    left, rem = self._traverse_expression_tree(E[i + 1 : end], 0, eval_all=True, debug=debug)
                    if left is None:
                        return None, rem
                    i = end + 1
                    if not eval_all:
                ...
                op = E[i]
                i = i + 1
            elif ((left is None) and (op in self.operators_unary) and (right is None)) or (
                ...
                if E[i] in (self.operators_nullary + self.operators_unary) or E[i] == "(":
                    right, i = self._traverse_expression_tree(E, i, eval_all=not self.left_associativity, debug=debug)
                    if right is None:
                        return None, i
                else:
        ...
        if (op is not None) or (right is not None):
            return None, "expression ended unexpectedly"
        if left is None:
    ...
    def evaluate(self, expression: str):
        ...
        def _split_tokens(expr):
            ...
            return [t.strip() for p in parts for t in re.split(r"\s+", p.strip()) if t.strip() != ""]

        # parse expression into tokens
        E = _split_tokens(expression)
        val, rem = self._traverse_expression_tree(E, debug=self.debug)
        if val is None:
            raise Exception(f"failed to evaluate expression: '{expression}', reason: {rem}")
