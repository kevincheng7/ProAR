            ...
            for var in ["X", "C", "S", "O"]:
                try:
                    ...
                    if not sequence and var in ["S", "O"]:
                        ...
                        raise Exception(f"Variable {var} is required by validation but not defined!")
            if tensors["X"] is not None and tensors["C"] is not None:
                if tensors["X"].shape[:2] != tensors["C"].shape[:2]:
                    raise ValueError(f"X shape {tensors['X'].shape} does not match C shape" f" {tensors['C'].shape}")
