...
class ScaledDotProductAttention(nn.Module):
    r"""Scaled dot product attention as described in Eqn 1 of Vaswani et al. 2017 [https://arxiv.org/abs/1706.03762].
...
class AttentionChainPool(nn.Module):
    ...
    def __init__(self, n_head, d_model):
        super().__init__()
        self.attention = MultiHeadAttention(n_head, d_model, d_model, d_model, dropout=0.0)
    ...
    def forward(self, h, C):
        bs, num_res = C.size()
        chains = C.abs().unique()
        chains = chains[chains > 0].unsqueeze(-1).repeat(1, bs).reshape(-1).unsqueeze(-1)
        ...
        output, _ = self.attention(self.get_query(h_repeat), h_repeat, h_repeat, mask=mask)
...
class Attention(nn.Module):
    ...
    def _check_inputs(self, Q, K, V, bias, mask):
        ...
        if d_q_in != self.d_model:
            raise ValueError("Dimension of Q tensor needs to be (batch_size, number_queries, d_model)")

        if d_k_in != self.d_model:
            raise ValueError("Dimension of K tensor needs to be (batch_size, number_keys, d_model)")

        if d_v_in != self.d_model:
            raise ValueError("Dimension of V tensor needs to be (batch_size, number_values, d_model)")

        if num_keys != num_values:
            raise ValueError("Number of keys needs to match number of values passed")

        if (batch_size_q != batch_size_k) or (batch_size_k != batch_size_v):
            raise ValueError("Found batch size mismatch among inputs, all tensors must agree in size of dimension 0")
        ...
        if bias is not None:
            ...
            if bias.dim() == 3:
                ...
                if num_queries_b != num_queries:
                    raise ValueError(
                        "Bias specified but number of queries (dim of axis=1) does not match number of queries given in Q tensor"
                    )
                if num_keys_b != num_keys:
                    raise ValueError(
                        "Bias specified but number of keys (dim of axis=2) does not match number of queries given in K tensor "
                        "(dimenson of axis=1)"
                    )
            elif bias.dim() == 4:
                if bias.dim() == 3:
        ...
        if mask is not None:
            ...
            if (num_queries_b != num_queries) and (num_queries_b != 1):
                raise ValueError(
                    "Bias specified but number of queries (dim of axis=2) does not match number of queries given in Q tensor"
                )
            if (num_keys_b != num_keys) and (num_keys_b != 1):
                raise ValueError(
                    "Bias specified but number of keys (dim of axis=3) does not match number of queries given in K tensor "
                    "(dimenson of axis=1)"
                )
