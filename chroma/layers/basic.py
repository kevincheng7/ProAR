...
class DropNormLin(nn.Module):
    ...
    def __init__(self, in_features, out_features, norm_type="ln", dropout=0.0, actn=nn.ReLU()):
    ...
    def forward(self, x, input_mask=None):
        ...
        if isinstance(self.norm_layer, MaskedBatchNorm1d):
            h = self.norm_layer(h.transpose(1, 2), input_mask=input_mask).transpose(1, 2)
        else:
            h = self.norm_layer(h)
...
class TriangleMultiplication(nn.Module):
    def __init__(self, d_model=512, mode="outgoing"):
        ...
        self.equation = "bikc,bjkc->bijc" if self.mode == "outgoing" else "bkjc,bkic->bijc"
        self.layer_norm = nn.LayerNorm(d_model)
        self.left_edge_mlp = nn.Sequential(nn.Linear(d_model, d_model), nn.Sigmoid(), nn.Linear(d_model, d_model))
        self.right_edge_mlp = nn.Sequential(nn.Linear(d_model, d_model), nn.Sigmoid(), nn.Linear(d_model, d_model))
...
class FourierFeaturization(nn.Module):
    ...
    def __init__(self, d_input, d_model, trainable=False, scale=1.0):
        ...
        if d_model % 2 != 0:
            raise ValueError("d_model needs to be even for this featurization, try again!")
