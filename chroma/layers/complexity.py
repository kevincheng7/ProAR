...
def compositions(S: torch.Tensor, C: torch.LongTensor, w: int = 30):
    ...
    edge_idx = torch.arange(S.shape[1], device=S.device)[None, :, None] + kx[None, None, :]
...
def complexity_lcp(
    ...
    if differentiable and len(S.shape) == 3:
        ...
        N_ij = (N_neighbors - S[:, :, None, :])[..., None, :] + torch.eye(N.shape[-1], device=N.device)[
            None, None, None, ...
        ]
...
def estimate_entropy(N: torch.Tensor, method: str = "chao-shen", eps: float = 1e-11) -> torch.Tensor:
