...
class GraphNN(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            node_h, edge_h = self.checkpoint(layer, node_h, edge_h, edge_idx, mask_i, mask_ij)
    ...
    def sequential(
        ...
        tensors["node_h_cache"], tensors["edge_h_cache"] = self.init_steps(tensors["node_h"], tensors["edge_h"])
    ...
    def init_steps(self, node_h: torch.Tensor, edge_h: torch.Tensor) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
    ...
    def step(
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Update edge and node
            node_h_t, edge_h_t = checkpoint(layer.step, t, node_h, node_h_out, edge_h, edge_idx, mask_i, mask_ij)

            # Scatter them in place
            node_h_cache[i + 1].scatter_(1, (t * torch.ones_like(node_h_t)).long(), node_h_t)
            edge_h_cache[i + 1].scatter_(1, (t * torch.ones_like(edge_h_t)).long(), edge_h_t)
...
class GraphLayer(nn.Module):
    ...
    def __init__(
        ...
        self.node_norm_layer = MaskedNorm(dim=1, num_features=dim_nodes, affine=True, norm=norm)
        ...
        self.edge_norm_layer = MaskedNorm(dim=2, num_features=dim_edges, affine=True, norm=norm)
        if self.edge_update:
            self.edge_mlp = MLP(
                dim_in=2 * dim_nodes + dim_edges,
                dim_out=dim_edges,
    ...
    def attend(self, node_h: torch.Tensor, messages: torch.Tensor, mask_ij: torch.Tensor) -> torch.Tensor:
    ...
    def _normalize_t(self, edge_node_stack_t, mask_ij_t, include_nodes=True, include_edges=True):
    ...
    def _update_nodes(self, node_h, node_h_norm, edge_h_norm, edge_idx, mask_i=None, mask_ij=None):
    ...
    def _update_nodes_t(
        ...
        # Compute messages at each ij
        edge_node_stack_t = mask_ij_t.unsqueeze(-1) * pack_edges_step(t, node_h, edge_h_norm_t, edge_idx_t)

        # Apply normalization of gathered tensors
        edge_node_stack_t = self._normalize_t(edge_node_stack_t, mask_ij_t, include_edges=False)
    ...
    def _update_edges_t(self, t, edge_h_t, node_h_out, edge_h_t_norm, edge_idx_t, mask_ij_t):
        """Update edges given adjacent nodes and edges"""
    ...
    def forward(
        ...
        node_h_out = self._update_nodes(node_h, node_h_norm, edge_h_norm, edge_idx, mask_i, mask_ij)
        edge_h_out = None
        if self.edge_update:
            edge_h_out = self._update_edges(edge_h, node_h_out, edge_h_norm, edge_idx, mask_ij)
    ...
    def step(
        ...
        """
        node_h_t_norm, edge_h_t_norm = self._normalize(node_h_t, edge_h_t, mask_i_t, mask_ij_t)
        node_h_t = self._update_nodes_t(t, node_h, node_h_t_norm, edge_h_t_norm, edge_idx_t, mask_i_t, mask_ij_t)

        if self.edge_update:
            node_h_out = node_h_out.scatter(1, (t * torch.ones_like(node_h_t)).long(), node_h_t)
            edge_h_t = self._update_edges_t(t, edge_h_t, node_h_out, edge_h_t_norm, edge_idx_t, mask_ij_t)
...
def collect_edges(edge_h_dense: torch.Tensor, edge_idx: torch.LongTensor) -> torch.Tensor:
...
def collect_edges_transpose(
    ...
    edge_h_transpose = edge_h_transpose.reshape(num_batch, num_residues, num_k, num_features)
...
def pack_edges(node_h: torch.Tensor, edge_h: torch.Tensor, edge_idx: torch.LongTensor) -> torch.Tensor:
...
def transpose_edge_idx(edge_idx: torch.LongTensor, mask_ij: torch.Tensor) -> Tuple[torch.LongTensor, torch.Tensor]:
    ...
    edge_idx_flat = edge_idx.reshape([num_batch, num_residues * num_k, 1]).expand(-1, -1, num_k)
    edge_idx_neighbors = torch.gather(edge_idx, 1, edge_idx_flat)
    # (b,i,j,k) gives the kth neighbor of the jth neighbor of i
    edge_idx_neighbors = edge_idx_neighbors.reshape([num_batch, num_residues, num_k, num_k])

    # 2. Determine which k at j maps back to i (if it exists)
    residue_i = torch.arange(num_residues, device=edge_idx.device).reshape((1, -1, 1, 1))
...
def permute_tensor(tensor: torch.Tensor, dim: int, permute_idx: torch.LongTensor) -> torch.Tensor:
...
class MaskedNorm(nn.Module):
    ...
    def forward(self, data: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if (mask is None) or (self.norm_type == "transformer"):
            ...
            mask_std = torch.sqrt((((data - mask_mean)).pow(2)).mean(dim=dims, keepdim=True) + self.eps)
            ...
            for mask_id in mask.unique():
                ...
                mask_std = torch.sqrt(
                    (((data - mask_mean) * tmask).pow(2)).sum(dim=dims, keepdim=True) / mask_sum + self.eps
                )
