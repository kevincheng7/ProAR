...
class MaskedBatchNorm2d(nn.Module):
    ...
    def forward(self, input, mask=None):
        ...
        if mask is not None:
            if mask.dim() != 4:
                raise ValueError(f"Input mask must have four dimensions, but has {mask.dim()}")
            b, l, l, d = mask.size()
            if (b != B) or (l != L):
                raise ValueError(f"Input mask must have shape {(B, L, L, 1)} or {(B, L, L, C)} to match input.")
            if d == 1:
        ...
        current_mean = masked_sum / n
        current_var = (mask * (masked - current_mean).pow(2)).sum(dim=(0, 1, 2), keepdim=True) / n
        # Update running stats
        with torch.no_grad():
            if self.track_running_stats and self.training:
                if self.num_batches_tracked == 0:
