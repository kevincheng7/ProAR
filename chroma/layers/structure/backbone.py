...
class RigidTransformer(nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            ...
            shape_X = list(X.size())
            shape_mask_expand = shape_mask + [1 for i in range(len(shape_X) - 1 - len(shape_mask))]
...
class BackboneBuilder(nn.Module):
    ...
    def forward(
        ...
        u_i_minus_2 = torch.tensor([[1.0, 0, 0]] * N_batch, dtype=torch.float32).to(device)
        u_i_minus_1 = torch.tensor([[0, 1.0, 0]] * N_batch, dtype=torch.float32).to(device)

        # Build chain via NERF recurrence
        X = []
        for i, (v_i, l_i) in enumerate(zip(v_list, lengths_list)):
            x_i, u_i_minus_1, u_i_minus_2 = _build_x_i(v_i, l_i, x_i, u_i_minus_1, u_i_minus_2)
            X.append(x_i)
        X = torch.stack(X, 1)
        # [N,AL,3] => [N,L,A,3]
...
class FrameBuilder(nn.Module):
    ...
    def forward(
        ...
        if R is None:
            ...
            R = geometry.rotations_from_quaternions(q, normalize=True, eps=self.distance_eps)
    ...
    def inverse(self, X: torch.Tensor, C: torch.LongTensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
class GraphBackboneUpdate(nn.Module):
    ...
    def _init_black_hole(self, X):
        R = torch.eye(3, device=X.device, dtype=X.dtype).reshape(1, 1, 3, 3).repeat(X.size(0), X.size(1), 1, 1)
    ...
    def _update_local_transform(self, X, C, node_h, edge_h, edge_idx, mask_i, mask_ij):
        ...
        # Predict transforms
        R = geometry.rotations_from_quaternions(self.W_q(node_h), normalize=True, eps=self.distance_eps)
    ...
    def _update_neighbor_transform(self, X, C, node_h, edge_h, edge_idx, mask_i, mask_ij):
        ...
        R_ji = geometry.rotations_from_quaternions(self.W_q(edge_h), normalize=True, eps=self.distance_eps)
    ...
    def _update_neighbor_global_transform(self, X, C, node_h, edge_h, edge_idx, mask_i, mask_ij):
        """Update residue frames via weighted average transformation from neighbors."""
        ...
        R_global, t_global = transforms.average_transforms(R_i, t_i, mask_i[..., None], mask_i, dim=1, dither_eps=0.0)
    ...
    def _update_neighbor_global_affine_transform(self, X, C, node_h, edge_h, edge_idx, mask_i, mask_ij):
        """Update residue frames via weighted average transformation from neighbors."""
        ...
        R_j_init, t_j_init = transforms.collect_neighbor_transforms(R_i_init, t_i_init, edge_idx)
        ...
        t_gi = s_node[1] * t_gi_init + (1.0 - s_node[1]) * d_scale * self.W_t_global(node_h)
        logit_gi = self.W_w_global(node_h)

        R_ji = geometry.rotations_from_quaternions(q_ji, normalize=True, eps=self.distance_eps)
        R_gi = geometry.rotations_from_quaternions(q_gi, normalize=True, eps=self.distance_eps)
    ...
    def _transform_loss(self, R_ij_predict, t_ij_predict, X, C, edge_idx, mask_ij):
        ...
        R_ij_mse = (mask_ij * R_ij_error).sum([1, 2]) / (mask_ij.sum([1, 2]) + self._eps)
        t_ij_mse = (mask_ij * t_ij_error).sum([1, 2]) / (mask_ij.sum([1, 2]) + self._eps)
    ...
    def forward(
        ...
        X_update, R_ji, t_ji, logit_ji = method(X, C, node_h, edge_h, edge_idx, mask_i, mask_ij)
        ...
        if self.unconstrained:
            ...
            if self.training:
                ...
                mask_drop = (torch.randint(low=0, high=2, size=[C.shape[0], 1, 1, 1], device=X.device) > 0).float()
                dX = mask_drop * dX
...
class LossBackboneResidueDistance(nn.Module):
    ...
    def _D(self, X):
        ...
        X_mean = X.mean(2)
        D = (X_mean[:, :, None, :] - X_mean[:, None, :, :]).square().sum(-1).add(self.dist_eps).sqrt()
        return D

    def forward(self, X_mobile: torch.Tensor, X_target: torch.Tensor, C: torch.LongTensor) -> torch.Tensor:
        mask = (C > 0).float()
...
def atomic_mean(X_flat: torch.Tensor, mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
def scale_around_mean(X: torch.Tensor, C: torch.LongTensor, scale: float) -> torch.Tensor:
    ...
    """
    num_atoms = X.size(2)
    mask_expand = (C > 0).float().reshape(list(C.shape) + [1, 1]).expand([-1, -1, num_atoms, -1])
    X_mean = (mask_expand * X).sum([1, 2], keepdims=True) / (mask_expand.sum([1, 2], keepdims=True))
