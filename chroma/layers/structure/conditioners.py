        ...
        for _conditioner in self.conditioners:
            if hasattr(_conditioner, "_postprocessing_"):
                if output_dict is None:
                    ...
                    protein, output_dict = _conditioner._postprocessing_(protein, output_dict)
...
class SubsequenceConditioner(Conditioner):
    ...
    def _transform_gradient(self, grad, C, t):
        ...
        scale = self.weight / self.design_model.noise_perturb.noise_schedule.sigma(t).to(C.device)
...
class ShapeConditioner(Conditioner):
    ...
    def __init__(
        ...
        if self.gw_layout:
            self._map_gw_coupling_ideal_glob(X_target, num_residues=autoscale_num_residues)
    ...
    def _distance_knn(self, X, top_k=12, max_scale=10.0):
        ...
        D = np.sqrt(((X_np[:, :, np.newaxis, :] - X_np[:, np.newaxis, :, :]) ** 2).sum(-1))
    ...
    def forward(
        ...
        scale_t = self.shape_loss_weight * self.noise_schedule.SNR(t).sqrt().clamp(min=1e-3, max=3.0)
...
class ProCapConditioner(Conditioner):
    ...
    def __init__(
        ...
        if isinstance(model, ProteinCaption):
            ...
            self.model = procap.load_model(model, device=device, strict_unexpected=False)
        self.model.eval()
        if device is None:
            if torch.cuda.is_available():
                self.model.to("cuda")
...
class ProClassConditioner(Conditioner):
    ...
    * pfam: protein family annotations which represent domain level structural
    ...
    def _proclass_neglogp(self, X, C, t, label, value=None, O=None, mask=None):
        ...
        if level == "chain":
            index = self.proclass_model.class_config[label]["tokenizer"][value] if value is not None else 0
            neglogp = neglogp[..., index][c_mask].sum()
        elif level == "first_order":
            if isinstance(value, str):
                index = torch.LongTensor([self.proclass_model.class_config[label]["tokenizer"][v] for v in value]).to(
                    neglogp.device
                )
                neglogp = torch.gather(neglogp, 2, index.unsqueeze(0).unsqueeze(2)).sum()
            elif isinstance(value, torch.Tensor):  # Mask Tensor is Passed for SS Conditioning
                logp = -1 * neglogp
                masked_log_probs = torch.where(value > 0, logp, -float("inf") * torch.ones_like(logp))
                log_probs_sum = torch.logsumexp(masked_log_probs, dim=-1)
                log_probs_sum = torch.where(value.sum(-1) > 0, log_probs_sum, torch.zeros_like(log_probs_sum))
    ...
    def _transform_gradient(self, grad, C, t):
        ...
        if grad.norm() > 1e-8:  # Don't rescale zero gradients!
            ...
            if self.renormalize_grad:
                scale = self.weight / self.proclass_model.noise_perturb.noise_schedule.sigma(t).to(C.device)
    ...
    def forward(
        ...
        neglogp = self._proclass_neglogp(X_input, C, t, self.label, value=self.value, O=O)
        if self.debug:
...
class SubstructureConditioner(Conditioner):
    ...
    def __init__(
        ...
        print("Note: We recommend using sde_func='langevin' for sampling with SubstructureCondtioner")
    ...
    def _rg_loss(self, X0, C):
        ...
        loss = self.base_distribution._multiply_R_inverse(X_target - X0, C).square().sum()
        return loss
...
class SymmetryConditioner(Conditioner):
    ...
    def __init__(
        ...
        print("Note: We recommend using sde_func='langevin' for sampling with SymmetryConditioner")

    def flat_bottom_potential(self, r, r0, k, d):
        condition = torch.abs(r - r0) < d
        return torch.where(condition, torch.zeros_like(r), k * (torch.abs(r - r0) - d) ** 2)
        ...
        if scale:
            if self.enable_rigid_drift:
                translate_ratio = self.translational_scaling(C) / (
                    self.translational_scaling(C_au) * (self.num_chain_neighbors + 1) ** 0.5
                )

            else:
                translate_ratio = 1.0

            mask_expand = (C_au > 0).float().reshape(list(C_au.shape) + [1, 1]).expand([-1, -1, n_atoms_per_res, -1])
            X_com = (mask_expand * X).sum([1, 2], keepdims=True) / (mask_expand.sum([1, 2], keepdims=True))
    ...
    def _postprocessing_(
        ...
        if output_dict is None:
            ...
            traj_sym, C_sym, S_sym = self.symmetrize_output(trajectory.to_XCS_trajectory()[0], C, S)
    ...
    def center_X(self, X, C):
        ...
        X_mean = (mask_expand * X[:, :, :4, :]).sum([1, 2], keepdims=True) / (mask_expand.sum([1, 2], keepdims=True))
        X_centered = X - X_mean
    ...
    def symmetrize_output(self, X, C, S):
        ...
        n_chains = self.num_chain_neighbors + 1 if self.num_chain_neighbors + 1 < self.G.shape[0] else self.G.shape[0]
    ...
    def forward(
        ...
        if self.canonicalize:
            X_canonical = torch.einsum("ij,barj->bari", self.G[seed_idx].inverse(), X_subdomain)
        else:
        ...
        def grad_surgery(dx):
            if self.grad_com_surgery:
                # inflate COM signal
                translate_ratio = self.translational_scaling(C_tess) / (self.translational_scaling(C))
        ...
        O_subdomain = (
            O[:, None, :, :].expand([-1, symmetry_order, -1, -1]).reshape(list(C_subdomain.shape) + [O.shape[-1]])
        )
...
class ScrewConditioner(Conditioner):
    ...
    def forward(self, X, C, O, U, t):
        ...
        O_screw = O[:, None, :, :].expand([-1, symmetry_order, -1, -1]).reshape(list(C_screw.shape) + [O.shape[-1]])
...
class RgConditioner(Conditioner):
    ...
    def means_per_chain(self, _X, _C, eps=1e-5):
        ...
        X_mean_chains = X_masked.sum([1, 3], keepdims=True) / (4 * mask_chains_expand.sum([1, 3], keepdims=True) + eps)
    ...
    def compute_Rg(
        ...
        X.requires_grad = True
        X_masked, X_mean_chains, X_mean_complex, mask_chains = self.means_per_chain(X, C)
