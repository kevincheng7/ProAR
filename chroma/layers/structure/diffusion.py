...
class GaussianNoiseSchedule:
    ...
    def g(self, t: Union[float, torch.Tensor]) -> torch.Tensor:
        ...
        if self.kind == "log_snr":
            ...
            g_sq = 2 * (sigma_deriv - alpha_prime_div_alpha * self.sigma(t)) * self.sigma(t)
            g = g_sq.sqrt()
    ...
    def linear_logsnr_grid(self, N: int, tspan: Tuple[float, float]) -> torch.Tensor:
        ...
        logsnr_noise = GaussianNoiseSchedule(kind="log_snr", log_snr_range=self.log_snr_range)
...
class NoiseTimeEmbedding(nn.Module):
    ...
    def forward(self, t: torch.Tensor, log_alpha: Optional[torch.Tensor] = None) -> torch.Tensor:
        if not isinstance(t, torch.Tensor):
            t = torch.Tensor([t]).float().to(self.fourier_features.B.device)
        if t.dim() == 0:
            t = t[None]

        h = {"t": lambda: t, "log_snr": lambda: self.noise_schedule.log_SNR(t)}[self.feature_type]()
...
class DiffusionChainCov(nn.Module):
    ...
    def reverse_sde(
        ...
        f = beta * (-1 / 2) * backbone.center_X(X, C) - g.pow(2) * score_scale_t * score_transformed
    ...
    def ode(
        ...
        # Impute missing data
        X = backbone.center_X(X, C)
        score = self.score(X, X0_func, C, t, conditioner, align_X0=align_X0, detach_X0=detach_X0)
        score_transformed = self.base_gaussian.multiply_covariance(score, C)
    ...
    def score(
        ...
        with torch.enable_grad():
            ...
            U_diffusion = self.energy(Xt, X0_func, Ct, t, detach_X0=detach_X0, align_X0=align_X0)
    ...
    def elbo(self, X0_pred, X0, C, t):
        ...
        loss_itd = (weight * (R_inv_dX.pow(2) - 1 / (1 + snr)) - 0.5 * np.log(np.pi * 2.0 * np.e)).reshape(X0.shape)
    ...
    def _baoab_sample_step(
        ...
        def baoab_step(_x, p, t):
            ...
            _x_half = _x + g.pow(2) * self.base_gaussian.multiply_covariance(p_half, C) * dt / 2  # A
            p_half2 = c1 * p_half + c3 * (1 / g) * self.base_gaussian._multiply_R_inverse_transpose(Z, C)  # O
            _x = _x_half + g.pow(2) * self.base_gaussian.multiply_covariance(p_half2, C) * dt / 2  # A
        ...
        with torch.random.fork_rng():
            ...
            for t in tqdm(T.tolist()):
                ...
                elbo_t = -self.noise_schedule.SNR_derivative(t).to(X.device) * (
                    ((mask_atoms * (X_denoise - X) / 10.0) ** 2).sum([1, 2, 3]) / mask_atoms.sum([1, 2, 3])
                )
                elbo.append(elbo_t)
    ...
    def estimate_logp(
        ...
        def divergence(fn, x, t):
            ...
            fn_out, eps_J_prod = torch.autograd.functional.vjp(fn, (t, x), vec_eps, create_graph=False)
            eps_J_eps = (eps_J_prod[1] * vec_eps).reshape(x.shape[0], -1).sum(-1).unsqueeze(-1)
        ...
        if return_trace_t:
            ...
            return (logp_x1 + np.array(ddlogp).mean()) / ((C > 0).float().sum().item() * 4)
            ...
            for t in tqdm(T.tolist()):
                ...
                if grad_logprob_Y_func is not None:
                    with torch.random.fork_rng():
                        ...
                        sigma_square = self.noise_schedule.sigma(t).square().to(X.device)
                        dXhat = sigma_square * self.base_gaussian.multiply_covariance(grad, C)
    ...
    def conditional_X0(self, X0: torch.Tensor, score: torch.Tensor, C: torch.tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        X_update = sigma.pow(2).div(alpha)[..., None] * self.base_gaussian.multiply_covariance(score, C)
        return X0 + X_update
    ...
    def sample_conditional(
        ...
        sigma_ts = (self.noise_schedule.sigma(t).pow(2) - alpha_ts.pow(2) * self.noise_schedule.sigma(s).pow(2)).sqrt()
...
class ReconstructionLosses(nn.Module):
    ...
    def _loss_rmsd(self, losses, X0_pred, X, C, t, w=None, X_t_2=None):
        ...
        global_mse_normalized = w * self.loss_scale * rmsd_denoise.square() / (rmsd_noise.square() + self._loss_eps)
    ...
    def _loss_pseudoelbo(self, losses, X0_pred, X, C, t, w=None, X_t_2=None):
        ...
        losses["elbo_X"], losses["batch_pseudoelbo_X"] = self.noise_perturb.pseudoelbo(unaligned_mse, C, t)
    ...
    def _loss_pair(self, losses, X0_pred, X, C, t, w=None, X_t_2=None):
        ...
        pair_mse_normalized = (
            ...
            * (
                ...
                / ((mask_ij_pair * rmsd_pair_noise.square()).sum([1, 2]) + self._loss_eps)
            )
        )
    ...
    def _loss_neighborhood(self, losses, X0_pred, X, C, t, w=None, X_t_2=None):
        ...
        losses["batch_neighborhood_mse"] = self._batch_average(neighborhood_mse_normalized, C)
    ...
    def _loss_distance(self, losses, X0_pred, X, C, t, w=None, X_t_2=None):
        ...
        distance_mse_normalized = self.loss_scale * (
            w * (mask * distance_mse).sum(1) / ((mask * distance_mse_noise).sum(1) + self._loss_eps)
        )
    ...
    def estimate_metrics(
        ...
        with torch.random.fork_rng():
            ...
            for t in tqdm(T.tolist(), desc="Integrating diffusion metrics"):
                ...
                losses_t = {k: v for k, v in losses_t.items() if not k.startswith("batch_") and k != "rmsd_ratio"}
            ...
            metrics = {
                k: torch.stack(v, 0).mean(0) for k, v in metrics_samples.items() if isinstance(v[0], torch.Tensor)
            }
        if return_samples:
...
def _debug_viz_gradients(pml_file, X_list, dX_list, C, S, arrow_length=2.0, name="gradient", color="red"):
    """ """
    lines = [
        ...
        for model_ix, X in enumerate(X_list):
            ...
            for a_ix in range(4):
                for i in range(X.size(1)):
                    ...
                    lines = lines + [f"obj_1 = obj_1 + [CYLINDER] + {x_i} + {x_j} + [0.15]" " + color_1 + color_1"]
