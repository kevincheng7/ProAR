...
class Distances(nn.Module):
    ...
    def forward(self, X: torch.Tensor, mask: Optional[torch.Tensor] = None, dim: float = -2) -> torch.Tensor:
...
def extend_atoms(
    ...
    X_4 = X_3 + lengths * (cos_angle * n_1 + (sin_angle * sin_dihedral) * n_2 + (sin_angle * cos_dihedral) * n_3)
    return X_4
...
class InternalCoords(nn.Module):
    ...
    def forward(
        ...
        _angles = lambda Xi, Xj, Xk: np.pi - angles(Xi, Xj, Xk, distance_eps=self.distance_eps)
        _dihedrals = lambda Xi, Xj, Xk, Xl: dihedrals(Xi, Xj, Xk, Xl, distance_eps=self.distance_eps)
        ...
        mask_D, mask_A, mask_L = _pad_pack(mask_D, mask_A, mask_L, mask_O_D, mask_O_A, mask_O_L)
...
def quaternions_from_rotations(R: torch.Tensor, eps: float = 1e-3) -> torch.Tensor:
    ...
    magnitudes = _sqrt(1 + torch.stack([R00 - R11 - R22, -R00 + R11 - R22, -R00 - R11 + R22], -1))
...
def rotations_from_quaternions(q: torch.Tensor, normalize: bool = False, eps: float = 1e-3) -> torch.Tensor:
...
def hat(omega: torch.Tensor) -> torch.Tensor:
    ...
    index1 = torch.tensor([7, 2, 3], device=omega.device).expand(*target.size()[:-1], -1)
    index2 = torch.tensor([5, 6, 1], device=omega.device).expand(*target.size()[:-1], -1)
    return target.scatter(-1, index1, omega).scatter(-1, index2, -omega).reshape(*target.size()[:-1], 3, 3)
