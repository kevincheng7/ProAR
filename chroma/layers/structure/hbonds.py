...
class BackboneHBonds(nn.Module):
    ...
    def forward(
        ...
        _invD = lambda Xi, Xj: (Xi - Xj).square().sum(-1).add(self._eps).sqrt().reciprocal()
        U_ij = self._coefficient * (_invD(N_i, O_j) - _invD(N_i, C_j) + _invD(H_i, C_j) - _invD(H_i, O_j))
...
class LossBackboneHBonds(nn.Module):
    ...
    def forward(
        ...
        # Compute per complex
        recovery_local = (hb_current * hb_target_local).sum([1, 2]) / (hb_target_local.sum([1, 2]) + self._eps)
...
def _ij_distance(
    ...
    ix = torch.arange(C.shape[1], device=C.device)[None, :, None].expand(C.shape[0], -1, -1)
