...
class ProteinFeatureGraph(nn.Module):
    ...
    def __init__(
        ...
        self.node_layers = nn.ModuleList([_init_layer(node_dict, option) for option in self.node_features])
        ...
        self.edge_layers = nn.ModuleList([_init_layer(edge_dict, option) for option in self.edge_features])
        ...
        """
            ...
            dimensions.
        """
        self.node_linears = nn.ModuleList([nn.Linear(l.dim_out, self.dim_nodes) for l in self.node_layers])
        self.edge_linears = nn.ModuleList([nn.Linear(l.dim_out, self.dim_edges) for l in self.edge_layers])
        return
    ...
    def forward(
        ...
        if mask_ij is None or edge_idx is None:
            edge_idx, mask_ij = self.graph_builder(X, C, custom_D=custom_D, custom_mask_2D=custom_mask_2D)
    ...
    def _load_centering_params(self, reference_pdb: str):
        ...
        key = reference_pdb + ";" + json.dumps(self.node_features) + ";" + json.dumps(self.edge_features)
    ...
    def forward(
        ...
        if self.criterion != "knn":
            if self.random_min_local > 0:
                ...
                mask_2D_remaining = torch.ones_like(mask_2D).scatter(2, edge_idx_local, mask_ij_remaining)
...
class NodeRadii(nn.Module):
    ...
    def forward(
        ...
        X_center = (mask_i * X).sum([1, 2], keepdim=True) / mask_i.sum([1, 2], keepdim=True)
...
class Edge2mers(nn.Module):
    ...
    def forward(
        ...
        X_ij = X_ij.reshape(num_batch, num_residues, num_neighbors, 2 * num_atom_types, 3)
        if C is not None:
...
class EdgeOrientationChain(nn.Module):
    ...
    def __init__(self, feature: str = "log", distance_eps: float = 0.1, norm_eps: float = 1e-1):
    ...
    def forward(
        ...
        h_chain_to_chain = self._transformation_features(X_chain_i, X_chain_j, R_chain_i, R_chain_j, edge_idx)
        h_chain_to_node = self._transformation_features(X_chain_i, X_j, R_chain_i, R_j, edge_idx)
...
class EdgePositionalEncodings(nn.Module):
    ...
    def __init__(self, dim_embedding: int = 128, period_range: tuple = (1.0, 1000.0)):
        ...
        self.encoding = PositionalEncoding(d_model=dim_embedding, period_range=period_range)
...
class EdgeRandomFourierFeatures2mer(nn.Module):
    ...
    def __init__(
        ...
        with torch.random.fork_rng():
            ...
            self.vector_f = FourierFeaturization(3, dim_embedding, trainable=trainable, scale=scale)
            self.distance_f = FourierFeaturization(64, dim_embedding, trainable=trainable, scale=scale)

            self.use_quaternion = use_quaternion
            if self.use_quaternion:
                self.quat_f = FourierFeaturization(4, dim_embedding, trainable=trainable, scale=scale)
    ...
    def forward(
        ...
        R_ji, t_ji = transforms.compose_inner_transforms(R_j, t_j, R_i.unsqueeze(-3), t_i.unsqueeze(-2))
...
class EdgeCartesianCoords(nn.Module):
    ...
    def forward(
        ...
        X_j = X_j_flat.reshape([num_batch, num_residues, num_neighbors, 1, self.num_atom_types, 3])

        X_i = X.reshape([num_batch, num_residues, 1, self.num_atom_types, 1, 3])
        dX = X_j - X_i
        edge_h = self.scale_factor * dX.reshape([num_batch, num_residues, num_neighbors, -1])
        if C is not None:
            mask_i = chain_map_to_mask(C)
...
def _cgo_cylinder(X1, X2, radius=0.5, rgb=(0.0, 0.0, 1.0)):
    ...
    r1, g1, b1 = rgb
    r2, g2, b2 = rgb
    cgo_str = f"[ 9.0, {x1}, {y1}, {z1}, {x2}, {y2}, {z2}, {radius}, {r1}, {g1}, {b1}, {r2}," f" {g2}, {b2} ]"
    return cgo_str
