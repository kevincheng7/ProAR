...
class NodeChiRBF(nn.Module):
    ...
    def _featurize(self, chi, mask_chi=None):
        ...
        chi_bin_center = torch.arange(0, self.num_chi_bins, device=chi.device) * 2.0 * np.pi / self.num_chi_bins
        ...
        chi_features = chi_features.reshape([num_batch, num_residues, self.num_chi * self.num_chi_bins])
        return chi_features
...
class EdgeSidechainsDirect(nn.Module):
    ...
    def __init__(
        ...
        self.rff = torch.nn.Parameter(2.0 * np.pi / self.length_scale * torch.randn((3, self.num_fourier)))
    ...
    def _local_coordinates_t(self, t, X, C, S, edge_idx_t):
        ...
        C_j = graph.collect_neighbors(C.unsqueeze(-1), edge_idx_t).reshape([num_batch, num_neighbors])
        S_j = graph.collect_neighbors(S.unsqueeze(-1), edge_idx_t).reshape([num_batch, num_neighbors])
    ...
    def _featurize(self, U_ij, mask_atoms_ij):
        if self.basis_type == "rff":
            ...
            r_ff, theta_ff, phi_ff = [
                self._fourier_expand(h, self.fourier_order) for h in [r_ij_scale, theta_ij, phi_ij]
            ]
            # Radial envelope function
            r_envelope = mask_atoms_ij * torch.exp(-r_ij * r_ij / (2 * self.length_scale**2))

            # Tensor outer product
            bf_ij = torch.einsum("bika,bikar,bikat,bikap->bikartp", r_envelope, r_ff, theta_ff, phi_ff).reshape(
                list(r_ij.shape)[:3] + [-1]
            )
