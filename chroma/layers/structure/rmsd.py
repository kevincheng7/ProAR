...
class CrossRMSD(nn.Module):
    ...
    def forward(self, X_mobile, X_target):
        ...
        norms = (X_mobile**2).sum(dim=[-1, -2]).unsqueeze(1) + (X_target**2).sum(dim=[-1, -2]).unsqueeze(0)
    ...
    def pairedRMSD(
        ...
        norms = (X_mobile_center**2).sum(dim=[-1, -2]) + (X_target_center**2).sum(dim=[-1, -2])
        sqRMSD = torch.relu((norms - 2 * top_eig) / (num_atoms + self._eps))
        rmsd = torch.sqrt(sqRMSD)
...
class LossFragmentPairRMSD(nn.Module):
    ...
    def __init__(self, k=7, method="symeig", method_iter=50, graph_num_neighbors=30):
        ...
        self.k = k
        self.rmsd = CrossRMSD(method=method, method_iter=method_iter)
        self.graph_builder = protein_graph.ProteinGraph(num_neighbors=graph_num_neighbors)
    ...
    def forward(self, X_mobile, X_target, C, return_coords=False):
        ...
        X_pair_mobile, C_pair_mobile = self._collect_X_fragment_pairs(X_mobile, C, edge_idx)
        X_pair_target, C_pair_target = self._collect_X_fragment_pairs(X_target, C, edge_idx)

        mask = (C_pair_mobile > 0).float()

        rmsd, X_pair_mobile_align = self.rmsd.pairedRMSD(X_pair_mobile, X_pair_target, mask, compute_alignment=True)
        if return_coords:
...
class LossNeighborhoodRMSD(nn.Module):
    ...
    def _collect_X_neighborhood(self, X, C, edge_idx):
        ...
        X_neighborhood = graph.collect_neighbors(X_flat, edge_idx).reshape([num_batch, num_nodes, -1, 3])
        C_neighborhood = graph.collect_neighbors(C_flat, edge_idx).reshape([num_batch, num_nodes, -1])
    ...
    def forward(self, X_mobile, X_target, C, return_coords=False):
        ...
        # Build graph and pair fragments
        edge_idx, mask_ij = self.graph_builder(X_target, C)
        X_neighborhood_mobile, C_neighborhood_mobile = self._collect_X_neighborhood(X_mobile, C, edge_idx)
        X_neighborhood_target, C_neighborhood_target = self._collect_X_neighborhood(X_target, C, edge_idx)
...
def rmsd_unaligned(X_a, X_b, mask=None, eps=1e-5, _min_rmsd=1e-8):
    ...
    if mask is None:
        ...
        rmsd = torch.sqrt((mask * squared_dev).sum(-1).clamp(min=_min_rmsd) / (mask.sum(-1) + eps))
    return rmsd
