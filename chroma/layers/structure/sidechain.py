...
class SideChainBuilder(nn.Module):
    ...
    def _init_maps(self):
        ...
        self.register_buffer("_chi_ix", 10 * torch.ones((self.num_chi, self.num_aa), dtype=torch.long))
    ...
    def forward(self, X, C, S, chi=None):
        ...
        # (B,L) => (B,AA,ATOM,L)
        S_expand4 = S.reshape([num_batch, 1, 1, num_residues]).expand(-1, 3, self.num_atoms, -1)
        ...
        if chi is not None:
            ...
            # (4,AA) => (B,AA,4)
            chi_ix_expand = self._chi_ix.unsqueeze(0).expand([num_batch, -1, -1]).transpose(-2, -1)
            # (B,AA,4) @ (B,L,4) => (B,L,4)
...
class ChiAngles(nn.Module):
    ...
    def forward(self, X, C, S):
        ...
        X_1, X_2, X_3, X_4 = X_chi.reshape([num_batch, num_residues, 4, 4, 3]).unbind(-2)
...
class LossSideChainRMSD(nn.Module):
    ...
    def _rmsd(self, X, X_target, atom_mask):
        ...
        rmsd = torch.sqrt(sd.sum(-1) / (atom_mask.sum(-1) + self.rmsd_eps) + self.rmsd_eps)
        return rmsd
...
class LossFrameAlignedGraph(nn.Module):
    ...
    def forward(self, X, X_target, C, S):
        if X_target.size(2) == 14:
            ...
            raise Exception("Size of atom dimension must be 4 (backbone) or 14 (all-atom).")

        # Build the union graph
        custom_mask_2D = None
        if self.interface_only:
            custom_mask_2D = torch.ne(C.unsqueeze(1), C.unsqueeze(2)).float()
        edge_idx_model, _ = self.graph_builder(X[:, :, :4, :], C, custom_mask_2D=custom_mask_2D)
        edge_idx_target, _ = self.graph_builder(X_target[:, :, :4, :], C, custom_mask_2D=custom_mask_2D)
...
class LossAllAtomDistances(nn.Module):
    ...
    def _dist_ij(self, X, edge_idx):
        ...
        D_ij = torch.sqrt(((X_ij.unsqueeze(-2) - X_ij.unsqueeze(-3)) ** 2).sum(-1) + self.distance_eps)
        return D_ij
    ...
    def forward(self, X, X_target, C, S):
        ...
        loss_i = (mask_ij * loss).sum([2, 3, 4]) / (mask_ij.sum([2, 3, 4]) + self.distance_eps)
        return loss_i
...
class LossSidechainClashes(nn.Module):
    ...
    def _dist_ij(self, X, edge_idx):
        ...
        D_ij = torch.sqrt(((X_i.unsqueeze(-2) - X_j.unsqueeze(-3)) ** 2).sum(-1) + self.distance_eps)
        return D_ij
    ...
    def forward(self, X, C, S, edge_idx=None, mask_j=None, mask_ij=None):
        ...
        if mask_ij is not None:
            mask_clash_ij = mask_clash_ij * mask_ij.reshape(list(mask_ij.shape) + [1, 1])
        ...
        if self.use_smooth_cutoff:
            bond_clash = mask_clash_ij * torch.sigmoid(self.smooth_cutoff_alpha * (D_clash_cutoff - D))
        else:
