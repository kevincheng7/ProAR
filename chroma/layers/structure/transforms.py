...
def compose_translation(R_a: torch.Tensor, t_a: torch.Tensor, t_b: torch.Tensor) -> torch.Tensor:
...
def collect_neighbor_transforms(
    ...
    R_j = graph.collect_neighbors(R_i_flat, edge_idx).reshape([num_batch, num_residues, num_neighbors, 3, 3])
...
def collect_neighbor_inner_transforms(
    ...
    R_ji, t_ji = compose_inner_transforms(R_j, t_j, R_i.unsqueeze(-3), t_i.unsqueeze(-2))
    return R_ji, t_ji
...
def equilibrate_transforms(
    ...
    for i in range(iterations):
        ...
        R_i, t_i = average_transforms(R_i_pred, t_i_pred, logit_ij, mask_ij, t_edge=t_edge, dim=2)
...
def average_transforms(
    ...
    """
    assert dim >= 0, "dimension must index from the left"
    w = torch.where(mask[..., None].bool(), w, torch.full_like(w, torch.finfo(w.dtype).min))
