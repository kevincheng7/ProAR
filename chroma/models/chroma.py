...
class Chroma(nn.Module):
    ...
    def sample(
        ...
        verbose: bool = False,
    ) -> Union[Union[Protein, List[Protein]], Tuple[Union[Protein, List[Protein]], dict]]:
        """
        ...
        if design_method is None:
            ...
            if isinstance(protein_sample, list):
                proteins = [self.design(protein, **design_kwargs) for protein in protein_sample]
            else:
        ...
        if (conditioner is not None) and hasattr(conditioner, "_postprocessing_"):
            proteins, output_dictionary = self._postprocess(conditioner, proteins, output_dictionary)
    ...
    def _postprocess(self, conditioner, proteins, output_dictionary):
        if output_dictionary is None:
            if isinstance(proteins, list):
                proteins = [conditioner._postprocessing_(protein) for protein in proteins]
            else:
            ...
            if isinstance(proteins, list):
                ...
                proteins, output_dictionary = conditioner._postprocessing_(proteins, output_dictionary)
        return proteins, output_dictionary
    ...
    def _sample(
        ...
        **kwargs,
        ...
        if not full_output:
            ...
            outs["S"] = S
            trajectories = self._format_trajectory(outs, "X_trajectory", trajectory_length)

            trajectories_Xhat = self._format_trajectory(outs, "Xhat_trajectory", trajectory_length)

            # use unconstrained C and S for Xunc_trajectory
            outs["S"] = S_unc
            outs["C"] = C_unc
            trajectories_Xunc = self._format_trajectory(outs, "Xunc_trajectory", trajectory_length)
    ...
    def _format_trajectory(self, outs, key, trajectory_length):
        trajectories = [
            Protein.from_XCS_trajectory(
                [outs_X[i][None, ...] for outs_X in self._resample_trajectory(trajectory_length, outs[key])],
                outs_C[None, ...],
                outs_S[None, ...],
            )
    ...
    def _resample_trajectory(self, trajectory_length, trajectory):
        if trajectory_length < 0:
            raise ValueError("The trajectory length must fall on the interval [0, sample_steps].")
        n = len(trajectory)
    ...
    def design(
        ...
        if design_method not in set(["potts", "autoregressive"]):
            raise NotImplementedError(
                "Valid design methods are potts and autoregressive, recieved" f" {design_method}"
            )
    ...
    def pack(self, protein: Protein, temperature_chi: float = 1e-3, clamped: bool = False) -> Protein:
    ...
    def score_backbone(
        ...
        if isinstance(proteins, list):
            metric_dictionary = [
                self._make_metric_dictionary(metrics, metrics_samples, idx=i) for i in range(len(proteins))
            ]
        else:
    ...
    def _protein_list_to_XCS(self, list_of_proteins, all_atom=False, device=None):
        ...
        Xs, Cs, Ss = zip(*[protein.to_XCS(all_atom=all_atom) for protein in list_of_proteins])
        ...
        with torch.no_grad():
            X = torch.cat([nn.functional.pad(X, (0, 0, 0, 0, 0, Dmax - X.shape[1])) for X in Xs])
    ...
    def _init_backbones(self, num_backbones, length_backbones):
        ...
        C = torch.cat([torch.full([rep], i + 1) for i, rep in enumerate(length_backbones)]).expand(X.shape[0], -1)
