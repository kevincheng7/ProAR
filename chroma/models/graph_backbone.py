...
class GraphBackbone(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.prediction_type.startswith("scale"):
            self.mlp_W = graph.MLP(dim_in=args.dim_nodes, num_layers_hidden=args.node_mlp_layers, dim_out=1)

        # Wrap sampling functions
        _X0_func = lambda X, C, t: self.denoise(X, C, t)
        self.sample_sde = lambda C, **kwargs: self.noise_perturb.sample_sde(_X0_func, C, **kwargs)
        self.sample_baoab = lambda C, **kwargs: self.noise_perturb.sample_baoab(_X0_func, C, **kwargs)
        self.sample_ode = lambda C, **kwargs: self.noise_perturb.sample_ode(_X0_func, C, **kwargs)
        self.estimate_metrics = lambda X, C, **kwargs: self.loss_diffusion.estimate_metrics(_X0_func, X, C, **kwargs)
        self.estimate_elbo = lambda X, C, **kwargs: self.noise_perturb.estimate_elbo(_X0_func, X, C, **kwargs)
        self.estimate_pseudoelbo_X = lambda X, C, **kwargs: self.noise_perturb.estimate_pseudoelbo_X(
            _X0_func, X, C, **kwargs
        )

    def _time_features(self, t):
        h = {"t": lambda: t, "log_snr": lambda: self.noise_schedule.log_SNR(t)}[self.time_feature_type]()
    ...
    def denoise(
        ...
        if time_h is None:
            time_h = torch.zeros([node_h.shape[0], 1, node_h.shape[2]], device=node_h.device)
        if self.prediction_type == "scale":
            ...
            skip = (1 - scale) * (ssnr > self.loss_scale_ssnr_cutoff).float().reshape(scale.shape)
    ...
    def _debug_plot_denoising_geometry(self, X, C, t=None):
        ...
        R_ji_native, t_ji_native = self.backbone_updates[0]._inner_transforms(X, C, edge_idx)
        R_ji_noise, t_ji_noise = self.backbone_updates[0]._inner_transforms(X_noise, C, edge_idx)
        R_ji_denoise, t_ji_denoise = self.backbone_updates[0]._inner_transforms(X_denoise, C, edge_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        X0_pred, R_ji_pred, t_ji_pred, logit_ji_pred, edge_idx, mask_ij = self.denoise(X_t, C, t, return_geometry=True)
        ...
        _importance_weight = lambda h: h * importance_weights.reshape([-1] + [1] * (len(h.shape) - 1))
        _weighted_avg = lambda h: (weights * _importance_weight(h)).sum() / (weights.sum() + self._loss_eps)
        ...
        if self.backbone_updates[0].method != "local":
            ...
            losses["batch_translate_mse"] = _weighted_avg(t_ij_mse / (self.loss_scale**2))
            losses["batch_rotate_mse"] = _weighted_avg(R_ij_mse)
            losses["batch_transform_mse"] = losses["batch_translate_mse"] + losses["batch_rotate_mse"]
