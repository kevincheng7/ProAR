...
class GraphClassifier(nn.Module):
        ...
        for label, config in class_config.items():
            ...
            if group != "second_order":
                if self.zero_grad_fix:
                    node_norm_layer = MaskedNorm(dim=1, num_features=dim_nodes, affine=True, norm="layer")
                    mlp = MLP(
                        dim_nodes,
                        dim_hidden=None,
                        dim_out=dim_out,
    ...
    @validate_XC()
    def gradient(self, X, C, t, label, mask=None, value=None, O=None, scale=1.0, max_norm=None):
        """
        Args:
