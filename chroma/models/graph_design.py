...
class GraphDesign(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder = BackboneEncoderGNN(
            ...
            num_layers=(args.num_layers if args.num_layers_encoder is None else args.num_layers_encoder),
    ...
    def forward(
        ...
        if self.kwargs["predict_S_potts"]:
            logp_S_potts = self.decoder_S_potts.loss(S, node_h, edge_h, edge_idx, mask_i, mask_ij)
        ...
        if self.sidechains:
            ...
            (
                ...
                _,
                mask_ij_causal,
            ) = self.decoder(X_noise, C, S, node_h, edge_h, edge_idx, mask_i, mask_ij, permute_idx)
        else:
            logp_S = (None,)
            logp_chi = None
            chi = None
        ...
        if self.separate_packing:
            ...
            node_h, edge_h = self.encoder_S_gnn(node_h, edge_h, edge_idx, mask_i, mask_ij)
    ...
    def encode(
        ...
        node_h, edge_h, edge_idx, mask_i, mask_ij = self.encoder(X, C, node_h_aux=node_h_aux)
    ...
    def predict_marginals(
        ...
        if not self.kwargs["predict_S_marginals"]:
            raise Exception("This version of GraphDesign was not trained with marginal prediction")
    ...
    def predict_potts(
        ...
        if not self.kwargs["predict_S_potts"]:
            raise Exception("This version of GraphDesign was not trained with Potts prediction")
        node_h, edge_h, edge_idx, mask_i, mask_ij = self.encode(X, C, t)
    ...
    def loss(
        ...
        **kwargs,
        ...
        o = self.forward(X, C, S, t=t, permute_idx=permute_idx, sample_noise=sample_noise)
        ...
        chi_sample = decoder.decoder_chi.sample(S, o["mask_chi"], o["node_h_chi"], o["mask_i"], temperature=0.01)
        ...
        clashes = self.loss_clash(X_sample, C, S, edge_idx=o["edge_idx"], mask_ij=o["mask_ij_causal"])
    ...
    def sample(
        ...
        logits_init = torch.zeros(list(C.shape) + [len(alphabet)], device=C.device).float()
        if ban_S is not None:
            ban_S = [alphabet.index(c) for c in ban_S]
        mask_sample, mask_sample_1D, S_init = potts.init_sampling_masks(logits_init, mask_sample, S=S, ban_S=ban_S)
        if not clamped:
            S = S_init
        ...
        if symmetry_order is not None and not (sampling_method == "potts"):
            raise NotImplementedError("Symmetric decoding is currently only supported for Potts models")

        if sampling_method == "potts":
            if not self.kwargs["predict_S_potts"]:
                raise Exception("This GraphDesign model was not trained with Potts prediction")
            ...
            if regularization == "LCP":
                C_complexity = C if symmetry_order is None else C[:, : C.shape[1] // symmetry_order]
        ...
        if return_scores:
            ...
            neglogp_S = -(mask_i * logp_S).sum([1]) / ((mask_i).sum([1]) + self.loss_eps)
            neglogp_chi = -(mask_chi * logp_chi).sum([1, 2]) / (mask_chi.sum([1, 2]) + self.loss_eps)
...
class BackboneEncoderGNN(nn.Module):
    ...
    def forward(
        ...
        node_h, edge_h, edge_idx, mask_i, mask_ij = self._checkpoint(self.feature_graph, X, C, edge_idx, mask_ij)
...
class SidechainDecoderGNN(nn.Module):
        ...
        **kwargs,
        ...
        if self.predict_chi:
            if self.sidechain_embedding == "chi_linear":
                ...
                self.embed_chi = NodeChiRBF(dim_out=args.dim_edges, num_chi=4, num_chi_bins=args.num_chi_bins)
    ...
    def forward(
        ...
        (
            ...
            mask_i_p,
            mask_ij_p,
        ) = graph.permute_graph_embeddings(node_h, edge_h, edge_idx, mask_i, mask_ij, permute_idx)
    ...
    def _decode_inner(self, X_p, C_p, S_p, chi_p, node_h_p, edge_h_p, edge_idx_p, mask_i_p, mask_ij_p):
        ...
        node_h_p, edge_h_p = self.gnn(node_h_p, edge_h_p, edge_idx_p, mask_i_p, mask_ij_p)
    ...
    def _decode_pre_func(self, t, tensors_t):
        ...
        if self.predict_chi:
            if self.sidechain_embedding in ["chi_rbf", "mixed_chi_X"]:
                h_chi_p_ij_t = graph.collect_neighbors(tensors_t["h_chi_p"], edge_idx_p_t)
    ...
    def _decode_post_func(
        ...
        if self.predict_chi:
            logp_chi_p_t, _ = self.decoder_chi(S_p_t, chi_p_t, mask_chi_p_t, node_h_p_t, mask_i_p_t)
...
class NodePredictorS(nn.Module):
    ...
    def __init__(
        ...
        self.training_loss = torch.nn.CrossEntropyLoss(reduction="none", label_smoothing=self.label_smoothing)
    ...
    def sample(
        ...
        if mask_S is not None:
            log_probs_S = torch.where(mask_S > 0, log_probs_S, -float("Inf") * torch.ones_like(log_probs_S))
        if top_p is not None:
            log_probs_S = _filter_logits_top_p(log_probs_S, p=top_p)
        p = torch.distributions.categorical.Categorical(logits=log_probs_S / temperature)
        S_sample = p.sample()
        return S_sample
...
class NodePredictorChi(nn.Module):
    ...
    def __init__(
        ...
        self.chi_embedding = nn.ModuleList(
            [NodeChiRBF(dim_out=dim_nodes, num_chi=i, num_chi_bins=num_chi_bins) for i in [1, 2, 3]]
        )
    ...
    def _init_chi_bins(self, num_chi_bins):
        # Setup bins
        bins = torch.tensor(np.linspace(-np.pi, np.pi, num_chi_bins + 1), dtype=torch.float32).reshape([1, 1, 1, -1])
    ...
    def _sample_continuous(self, logits, left, right):
        ...
        # Local CDF inversion
        z = Y_left + (Y_right - Y_left) * (u.unsqueeze(-1) - CMF_left) / (CMF_right - CMF_left + 1e-5)
        z = z.squeeze(-1)
        return z
    ...
    def forward(
        ...
        if self.training:
            ...
            chi_onehot = chi_onehot * (1 - self.label_smoothing) + (1 - chi_onehot) * scale
    ...
    def sample(
        ...
        num_batch, num_residues, _ = node_h.shape
        chi = torch.zeros([num_batch, num_residues, 4], dtype=torch.float32, device=node_h.device)
...
class ProteinTraversalSpatial(nn.Module):
    ...
    def __init__(
        ...
        self.norm_eps = 1e-5
        self.protein_graph = protein_graph.ProteinGraph(num_neighbors=graph_num_neighbors)
    ...
    def forward(
        ...
        if alpha > 0:
            ...
            for i in range(self.smooth_steps):
                ...
                z_average = (mask_ij * z_neighbors).sum(2) / (mask_ij.sum(2) + self.norm_eps)
