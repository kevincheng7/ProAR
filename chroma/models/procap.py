...
class ProteinCaption(nn.Module):
    ...
    def __init__(
        ...
        if self.standardize_context_embeddings:
            self.context_normalization = nn.LayerNorm(self.d_model, elementwise_affine=False)
        ...
        if self.use_transformer:
            ...
            if gnn_embed_ratio != 1:
                print("Warning: both use_transformer and gnn_embed_ratio are set, setting" " gnn_embed_ratio to 1")
                self.gnn_embed_ratio = 1
            if context_per_chain != 1:
                print("Warning: both use_transformer and context_per_chain are set," " setting context_per_chain to 1")
        ...
        if self.direct_gnn:
            if gnn_embed_ratio != 1:
                print("Warning: both direct_gnn and gnn_embed_ratio are set, setting" " gnn_embed_ratio to 1")
                self.gnn_embed_ratio = 1
            if context_per_chain != 1:
                print("Warning: both direct_gnn and context_per_chain are set, setting" " context_per_chain to 1")
                self.context_per_chain = 1
            if not self.only_encode_caption_chain:
                print(
                ...
                self.only_encode_caption_chain = True
            if self.use_transformer:
                print("Warning: direct_gnn and use_transformer are both set, turning off" " use_transformer")
                self.use_transformer = False
            if self.context_size is not None:
                print("Warning: context_size given but not used for direct_gnn, setting" " context_size to None")
        ...
        if self.classifier_checkpoint is not None or self.classifier_kwargs is not None:
            if self.classifier_kwargs is not None:
                ...
                self.protein_encoder = graph_classifier.load_model(classifier_checkpoint)
            ...
            self.protein_encoder_linear = nn.Sequential(
                nn.Linear(self.protein_encoder.dim_nodes, self.d_model * self.gnn_embed_ratio),
                nn.ReLU(),
            )
        else:
    ...
    def forward(
    ...
    ) -> Union["transformers.modeling_outputs.CausalLMOutputWithCrossAttentions", torch.Tensor]:
        ...
        if add_noise and self.noise_generator is not None:
            ...
            assert all(t <= 1) and all(t >= 0), (
                "Noise Temperatures must be between 0 and 1, but got values" f" {t[(t > 1) | (t < 0)]}"
            )
        else:
        ...
        if self.classifier_kwargs is None:
            ...
            if O is not None:
                ...
                node_h = node_h + pad(O, (0, 2)) @ self.sequence_embedding.weight.to(X.device)
        ...
        context_embedding, attention_mask_context = self._encode_context(Xe, C, Me, chain_id)
        if self.standardize_context_embeddings:
            context_embedding = self.context_normalization.to(Xe.device)(context_embedding)
        elif self.normalize_context_embeddings:
            context_embedding = torch.nn.functional.normalize(context_embedding, dim=-1)

        # Encode Text Input
        if self.direct_gnn:
            max_caption_tokens = self.tokenizer.model_max_length - context_embedding.shape[1]
        else:
            max_caption_tokens = (
                self.tokenizer.model_max_length
                - (self.context_size - 1) * self.gnn_embed_ratio * self.context_per_chain
                - 1
            )
        Y, attention_mask_caption = self._tokenize(caption, add_stop=True, max_length=max_caption_tokens)
        Y = Y.to(X.device)
        ...
        attention_mask = torch.cat([attention_mask_context, attention_mask_caption], dim=1)
        labels = torch.cat(
            [
                torch.tensor(-100, device=X.device).expand(attention_mask_context.shape),
        ...
        if not by_sample:
            ...
            return torch.Tensor(
                (loss * (shift_labels != -100).int()).sum(dim=-1) / (shift_labels != -100).int().sum(dim=-1)
            )

        return output

    @validate_XC(all_atom=False)
    def _noise(self, X: torch.Tensor, C: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _encode_context(
        ...
        for x, c, pid in zip(Xe, Cm, polymer_id):
            ...
            if pid == -1:
                pdb_embedding = self._embed_text(self._tokenize(["<|pdb|>"], add_stop=False)[0].to(Xe.device)).squeeze(
                    0
                )
            ...
            if pid == 0:
                pdb_embedding = (
                    self._embed_text(self._tokenize(["<|unconditioned|>"], add_stop=False)[0]).squeeze(0).to(Xe.device)
                )
                batch_encoding.append(pdb_embedding)
                ...
                if self.only_encode_caption_chain and (pid != -1):
                    ...
                    if self.use_transformer:
                        encodings = [
                            self.transformer.to(Xe.device)(
                                ...
                                torch.zeros(1, self.context_size, self.d_model).to(Xe.device),
                            ).squeeze(0)
                        ]
                        ...
                        encodings = [
                            (x[residue_mask].pow(p).sum(0).unsqueeze(0) / n_residues).abs().pow(1 / p)
                            * (x[residue_mask].pow(p).sum(0).unsqueeze(0).sign() if p % 2 == 1 else 1)
                            for p in range(1, self.context_per_chain + 1)
                        ]
                        encodings = [enc.reshape(self.gnn_embed_ratio, -1) for enc in encodings]
                    batch_encoding.extend(encodings)
                else:
                    if self.use_transformer or self.direct_gnn:
                        residue_mask = c > 0  # just use all embeddings, no chain structure
                        if self.use_transformer:
                            ...
                            assert self.only_encode_caption_chain, (
                                "only_encode_caption chain = False not supported when" " use_transformer = True!"
                            )
                            batch_encoding.append(
                                self.transformer.to(Xe.device)(
                                    x[residue_mask].unsqueeze(0),
                                    torch.zeros(1, self.context_size - 1, self.d_model).to(Xe.device),
                                ).squeeze(0)
                            )
                        else:  # direct_gnn
                        ...
                        for cid in torch.unique(c):
                            ...
                            batch_encoding.extend([enc.reshape(self.gnn_embed_ratio, -1) for enc in encodings])
                    ...
                    if pid != -1:
                        ...
                        try:
                            ...
                            print("Problem: tried to switch encodings at positions 0 and" f" {first_cid}, but failed!")
                            # raise
        ...
        if self.context_size is not None:
            if X_pooled.shape[1] > (self.context_size - 1) * self.gnn_embed_ratio * self.context_per_chain + 1:
                print([x.shape for x in pooled_encoding])
                print(polymer_id)
            assert X_pooled.shape[1] <= (self.context_size - 1) * self.gnn_embed_ratio * self.context_per_chain + 1, (
            ...
            if (
                X_pooled.shape[1] < (self.context_size - 1) * self.gnn_embed_ratio * self.context_per_chain + 1
                and not self.direct_gnn
            ):
                pad_shape = (
                    (self.context_size - 1) * self.gnn_embed_ratio * self.context_per_chain + 1 - X_pooled.shape[1]
                )
                zero_pad = torch.zeros([B, pad_shape, int(Dm / self.gnn_embed_ratio)], device=Xe.device)
...
def save_model(model: ProteinCaption, weight_file: str, metadata: Optional[dict] = None) -> None:
