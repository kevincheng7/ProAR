...
def letter_to_point_cloud(
    letter="G",
    width_pixels=35,
    font=os.path.join(os.path.dirname(chroma.__path__[0]), "assets/LiberationSans-Regular.ttf"),
    depth_ratio=0.15,
    fontsize_ratio=1.2,
    stroke_width=1,
    margin=0.5,
...
def point_cloud_rescale(X, num_residues, neighbor_k=8, volume_per_residue=128.57, scale_ratio=0.4):
...
def point_cloud_volume(X, radius):
    ...
    D = np.sqrt(np.square(X[None, :] - X[:, None]).sum(-1))
    overlap_ij = (D < 2.0 * radius) * (np.pi / 12.0) * (4.0 * radius + D) * (2.0 * radius - D) ** 2
...
def plane_split_protein(X=None, C=None, protein=None, mask_percent=0.5):
    ...
    normal = torch.from_numpy(pca.fit_transform(points.detach().cpu().numpy().transpose(1, 0))).to(X.device)
    c_alphas = X[:, :, 1, :]
    ...
    if tries >= 300000:
        print("Tried and failed to split protein by plane to grab" f" {mask_percent} residues.")
        c = 0
        C_mask = ((c_alphas @ normal) > c).squeeze(-1) & (C > 0)
        print(f"Returning {100 * percent_masked(0.0):.2f} percent residues masked" " instead.")

    else:
        C_mask = ((c_alphas @ normal) > c).squeeze(-1) & (C > 0)
        print(f"Split protein by plane, masking {100 * percent_masked(c):.2f} percent of" " residues.")
