...
def save_model(model, weight_file, metadata=None):
    ...
    local_path = str(Path(tempfile.gettempdir(), str(uuid4())[:8]) if weight_file.startswith("s3:") else weight_file)
...
def load_model(
    ...
    if str(weights).startswith("https:"):
        ...
        weights = api.download_from_generate(base_url, model_name, force=False, exist_ok=True)

    # load model weights
    params = torch.load(weights, map_location="cpu")
    model = model_class(**params["init_kwargs"]).to(device)
    missing_keys, unexpected_keys = model.load_state_dict(params["model_state_dict"], strict=strict)
