...
def num_known_molecular_tokens(ptype=polymerType.LPROT):
    return sum([not is_punctuation_index(idx) and not is_unknown(idx) for idx in range(len(_res3[ptype.value]))])
...
def is_canonical(res: str, ptype=polymerType.LPROT):
    if ptype == polymerType.LPROT or ptype == polymerType.DPROT:
        ...
        return is_canonical(res, polymerType.LPROT) or is_canonical(mirror_amino_acid(res), polymerType.DPROT)
...
for grp in [1, 2, 3]:
    for tp in [polymerType.LPROT, polymerType.DPROT]:
        for idx in range(num_tokens(tp)):
            if grp == 1:
                if not is_punctuation_index(idx, tp) and (not is_unknown_index(idx, tp)):
