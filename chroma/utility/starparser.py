...
def star_read_data_row(f, row: list, in_loop: bool, has_blocks: bool, peeked: PeekedLine = None):
    i = 0
    ret = True
    if peeked is None:
        peeked = PeekedLine("", 0)
    while i < len(row):
        ...
        if (
            ...
            if i == 0:
                advance(f, peeked)
                return False
            raise Exception(f"data block ended while reading requested number of tokens: {len(row)}")
        ...
        if peeked.line.startswith(";"):
            ...
            elems = [part for part in shlex.split(peeked.line.strip())] if has_blocks else peeked.line.strip().split()
