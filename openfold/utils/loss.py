...
from openfold.utils.tensor_utils import masked_mean, permute_final_dims, tensor_tree_map, tree_map
...
def compute_fape(
    ...
    pair_distance: Optional[torch.Tensor] = None,
    weight_params: Optional[list] = [2.0, 12.0, 1.5],
    residue_variance: Optional[torch.Tensor] = None,
    variance_weight_beta: float = 20.0,
    eps=1e-8,
) -> torch.Tensor:
    """
    Computes FAPE loss with optional weighting.

    This loss measures the deviation between predicted and target atomic positions
    after aligning them to their respective local rigid frames. It optionally supports:
        - distance-based weighting based on pairwise residue distances
        - per-residue variance-based weighting to down-weight flexible or uncertain regions
    ...
    Args:
        ...
        pair_distance:
            [*, N_frames, N_pts] optional pairwise distance matrix between frames and points,
            used to assign greater or lesser weight to position pairs based on their physical proximity
        weight_params:
            Optional list of 3 values [alpha, h_offset, v_offset] controlling the sigmoid-based
            distance weighting function:
                weight(d_ij) = 1 / (1 + exp(-alpha * (-d_ij + h_offset))) + v_offset
        residue_variance:
            [*, N_frames] optional tensor representing per-residue variance (e.g., from dynamics);
            higher variance leads to lower contribution to the loss
        variance_weight_beta:
            Scalar controlling the steepness of the variance decay function:
                variance_weight = 1 / (1 + beta * variance)
        eps:
    ...
    if l1_clamp_distance is not None:
        ...
        if pair_distance is not None and weight_params is not None:
            alpha, h_offset, v_offset = weight_params
            weights = 1.0 / (1.0 + torch.exp(-alpha * (-pair_distance + h_offset))) + v_offset
            if weights.shape[1:] != error_dist.shape[1:]:
                raise ValueError(f"weights shape {weights.shape} != error_dist shape {error_dist.shape}")
            error_dist = torch.where(error_dist < l1_clamp_distance, error_dist * weights, error_dist)

    if residue_variance is not None:
        # shape [*, N_frames, 1] for broadcasting
        decay_weights = 1.0 / (1.0 + variance_weight_beta * residue_variance)[..., None]
        error_dist = error_dist * decay_weights
...
def backbone_loss(
    ...
    **kwargs,
    ...
    pair_distance = torch.cdist(gt_aff.get_trans(), gt_aff.get_trans())
    ...
    fape_loss = compute_fape(
        ...
        eps=eps,
        pair_distance=pair_distance[None],
...
def sidechain_loss(
    ...
    **kwargs,
    ...
    pair_distance = torch.cdist(
        renamed_gt_frames.get_trans(), renamed_atom14_gt_positions
    )  # (bs, N*8 frames, N*14 pts)
    if residue_variance is not None:
        residue_variance = residue_variance[..., None].repeat(1, 1, 8).view(*batch_dims, -1)  # (bs, N*8 frames)
