...
from src.models.modules import conv
...
class BackboneMVNGlobular(torch.nn.Module):
    ...
    def _atomic_mean(self, X_flat, mask):
        ...
        X_mean = torch.sum(mask_atomic * X_flat, 1, keepdims=True) / (torch.sum(mask_atomic, 1, keepdims=True))
        return X_mean, mask_atomic

    def _C_atomic(self, C, expand_heavy_atom: bool = True):
        # Expand chain map into atomic level masking
        if expand_heavy_atom is True:
            C_expand = C.unsqueeze(-1).expand(-1, -1, 4)
        else:  # only expand CA atom
    ...
    def _globular_parameters(self, C_mask_all, translation_inflation=None):
        ...
        if translation_inflation is not None:
            ...
            std_correction = torch.sqrt(N_chains_per_complex / (N_chains_per_complex - 1).clamp(min=1))
    ...
    def _expand_masks(self, C, expand_heavy_atom: bool = True):
        C_atomic = self._C_atomic(C, expand_heavy_atom)
        C_mask_all = expand_chain_map(torch.abs(C_atomic))[..., None]
        C_mask_present = expand_chain_map(C_atomic)[..., None]
        return C_mask_all, C_mask_present

    def _expand_per_chain(self, Z, C, expand_heavy_atom: bool = True):
        """Build augmented [num_batch, 4*num_residues, num_chains, 3] system
        or [num_batch, num_residues, num_chains, 3] system when expand_heavy_atom=False."""
        # Build masks and augmented [B,4N,C,3] system
        C_mask_all, C_mask_present = self._expand_masks(C, expand_heavy_atom)
    ...
    def _shift_means(self, X_expand, C_mask_mean, C_mask_apply, scale, shift=None):
        ...
        X_chain_mean = (C_mask_mean * X_expand).sum(1, keepdims=True) / (C_mask_mean.sum(1, keepdims=True) + 1e-5)
    ...
    def _translate_by_x1(self, X_expand, C_mask, scale_mean, scale_x1):
        ...
        X_expand = self._shift_means(X_expand, C_mask, C_mask, scale=scale_mean, shift=X_1 * scale_x1)
        return X_expand
    ...
    def _multiply_R(self, Z, C, expand_heavy_atom: bool = True):
        """Multiply by the square root of the covariance matrix"""
        if Z.dim() == 4:
            Z = Z.reshape(Z.shape[0], -1, 3)

        C_mask_all, C_mask_present, Z_expand = self._expand_per_chain(Z, C, expand_heavy_atom)
        ...
        if self.covariance_model == "brownian":
            ...
            R_Z_expand = self._translate_by_x1(R_Z_expand, C_mask_all, scale_mean=-1, scale_x1=self.sigma_translation)
            ...
            Z_expand = (1.0 - mask_init) * Z_expand + mask_init * x_init_std[:, None, ...] * Z_expand
            ...
            R_Z_expand = self._shift_means(R_Z_expand, C_mask_all, C_mask_all, scale=scale)

        # Collapse out chain dimension
        if expand_heavy_atom is True:
            R_Z = R_Z_expand.sum(2).reshape(Z.shape[0], -1, 4, 3)
        else:
            R_Z = R_Z_expand.sum(2).reshape(Z.shape[0], -1, 1, 3)
        return R_Z

    def _multiply_R_wo_R_center(self, Z, C, expand_heavy_atom: bool = True):
        """Multiply by the square root of the covariance matrix"""
        if Z.dim() == 4:
            Z = Z.reshape(Z.shape[0], -1, 3)
    ...
    def _multiply_R_transpose(self, Z, C):
        ...
        if self.covariance_model == "brownian":
            ...
            scale = -mean_shift_scale[:, None, ...]
            Rt_Z_expand = self._shift_means(Rt_Z_expand, C_mask_all, C_mask_all, scale=scale)
            ...
            # [B,C,1] => [BC,1] => [BC,3] => [BC3]
            B_1D = B.reshape([-1, 1]).expand([-1, 3]).reshape([-1])
            Rt_Z_1D_reverse = self._scale * conv.filter1D_linear_decay(Z_1D_reverse, B_1D)
            Rt_Z_1D = torch.flip(Rt_Z_1D_reverse, [1])
            # [BC3,4N] -> [B,C,3,4N] -> [B,4N,C,3]
            Rt_Z_expand = Rt_Z_1D.reshape([num_B, num_C, 3, num_N]).permute([0, 3, 1, 2])
            ...
            mask_init = (1.0 - C_mask_prev) * C_mask_all
            # Inflate z_1 by the equilibrium variance
            Rt_Z_expand = (1.0 - mask_init) * Rt_Z_expand + mask_init * x_init_std[:, None, ...] * Rt_Z_expand
    ...
    def _multiply_R_inverse(self, X, C):
        ...
        if self.covariance_model == "brownian":
            ...
            X_expand = self._translate_by_x1(X_expand, C_mask_all, scale_mean=1 / self.sigma_translation, scale_x1=-1)
            ...
            Ri_X_expand = C_mask_all * (X_expand - B[:, None, ...] * X_prev) / self._scale
            ...
            C_mask_prev = F.pad(C_mask_all[:, :-1, :, :], (0, 0, 0, 0, 1, 0))
            mask_init = (1.0 - C_mask_prev) * C_mask_all
            Ri_X_expand = (1.0 - mask_init) * Ri_X_expand + mask_init * Ri_X_expand / x_init_std[:, None, ...]
    ...
    def _multiply_R_inverse_transpose(self, X, C):
        ...
        if self.covariance_model == "brownian":
            ...
            C_mask_prev = F.pad(C_mask_all[:, :-1, :, :], (0, 0, 0, 0, 1, 0))
            mask_init = (1.0 - C_mask_prev) * C_mask_all
            Rit_X_expand = (1.0 - mask_init) * Rit_X_expand + mask_init * Rit_X_expand / x_init_std[:, None, ...]

            # Step 2. R_sum_inverse_transpose
            # Apply linear recurrence `x_i = z_i + b * x_{i-1}`
            X_future = F.pad(Rit_X_expand[:, 1:, :, :], (0, 0, 0, 0, 0, 1))
            Rit_X_expand = C_mask_all * (Rit_X_expand - B[:, None, ...] * X_future) / self._scale

            # Step 3. R_center_inverse_transpose = R_center_inverse (by symmetry)
            # Rescale translational variance
            mean_shift_scale_inverse = mean_shift_scale / (1 - mean_shift_scale)
            scale = mean_shift_scale_inverse[:, None, ...]
            Rit_X_expand = self._shift_means(Rit_X_expand, C_mask_all, C_mask_all, scale=scale)

        Rit_X = Rit_X_expand.sum(2).reshape(X.shape[0], -1, 4, 3)
        return Rit_X

    def _materialize_RRt(self, C, expand_heavy_atom: bool = True):
        ...
        if expand_heavy_atom is True:
            ...
            Z = torch.randn(bs, sl, 3).to(C.device)
        C_mask_all, C_mask_present, Z_expand = self._expand_per_chain(Z, C, expand_heavy_atom)

        b, x_init_std, xi = self._globular_parameters(C_mask_all)

        C_atom = self._C_atomic(C.abs(), expand_heavy_atom)
    ...
    def _materialize_RRt_wo_R_center(self, C, expand_heavy_atom: bool = True):
        """As in C.4 of `https://www.biorxiv.org/content/10.1101/2022.12.01.518682v1.full.pdf`"""
        assert C.shape[0] == 1
        a = self._scale
        bs, sl = C.size()
    ...
    def _build_R_center(self, C_atom, xi):
        ...
        for chain_index, _xi in zip(chain_indices, xi[0]):
            ...
            blocks.append((torch.eye(N, device=_xi.device) - (_xi / N) * torch.ones(N, N, device=_xi.device)))
    ...
    def log_determinant(self, C):
        ...
        logdet_chain = N_chain * np.log(a) + torch.log(1.0 - xi) - 0.5 * torch.log(1.0 - B**2)
...
class ConditionalBackboneMVNGlobular(BackboneMVNGlobular):
    ...
    def _center_of_mass(self, X, C):
        mask_expand = (C > 0).float().reshape(list(C.shape) + [1, 1]).expand([-1, -1, 4, -1])
        X_mean = (mask_expand * X).sum([1, 2], keepdims=True) / (mask_expand.sum([1, 2], keepdims=True))
        return X_mean
    ...
    def _materialize_RRt(self, C):
        ...
        b, x_init_std, xi = self._globular_parameters(C_mask_all, translation_inflation=gamma)
    ...
    def _condition_RRt(self, RRt, D):
        ...
        RRt_clamp = self._scatter(torch.zeros_like(RRt), self.zero_indices, self.RRt_clamp_restricted)
...
class BackboneMVNResidueGas(torch.nn.Module):
    ...
    def __init__(self, stddev_CA=10.0, stddev_atoms=1.0, **kwargs):
        ...
        s2 = stddev_atoms
        # Atoms are N-CA-C=O
        R_local = torch.tensor([[s2, s1, 0, 0], [0, s1, 0, 0], [0, s1, s2, 0], [0, s1, 0, s2]]).float()
    ...
    def log_prob(self, X: torch.Tensor, C: torch.Tensor, *, normalized: bool = False) -> torch.Tensor:
