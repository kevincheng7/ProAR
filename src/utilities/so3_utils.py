...
def scale_rotmat(rotation_matrix: torch.Tensor, scalar: torch.Tensor, tol: float = 1e-7) -> torch.Tensor:
    """
...
def skew_matrix_exponential_map_axis_angle(angles: torch.Tensor, skew_matrices: torch.Tensor) -> torch.Tensor:
    """
    ...
    exp_skew = (
        ...
        + (1.0 - torch.cos(angles)) * torch.einsum("b...ik,b...kj->b...ij", skew_matrices, skew_matrices)
    )
    return exp_skew


def skew_matrix_exponential_map(angles: torch.Tensor, skew_matrices: torch.Tensor, tol=1e-7) -> torch.Tensor:
    """
...
def _rotquat_to_axis_angle(rotation_quaternions: torch.Tensor, tol: float = 1e-7) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    def save_cache(self, data: Dict[str, torch.Tensor]) -> None:
        ...
        if self.path_exists:
            if self.overwrite:
                ...
                logger.warn(f"Cache at {self.cache_path} exits and overwriting disabled. Doing nothing.")
        else:
...
class BaseSampleSO3(nn.Module):
    ...
    def expansion_function(self, omega_grid: torch.Tensor, sigma_grid: torch.Tensor) -> torch.Tensor:
        """
...
def igso3_expansion(omega: torch.Tensor, sigma: torch.Tensor, l_grid: torch.Tensor, tol=1e-7) -> torch.Tensor:
    """
    ...
    f_igso = torch.where(torch.logical_or(torch.isinf(f_igso), torch.isnan(f_igso)), torch.zeros_like(f_igso), f_igso)

    return f_igso


def digso3_expansion(omega: torch.Tensor, sigma: torch.Tensor, l_grid: torch.Tensor, tol=1e-7) -> torch.Tensor:
    """
    ...
    # Pre-compute numerator of expansion which only depends on angles.
    numerator_sin = l_grid[None, :] * torch.sin(l_fac_2[None, :] * omega[:, None]) - l_fac_2[None, :] * torch.sin(
        l_grid[None, :] * omega[:, None]
    )
...
def dlog_igso3_expansion(omega: torch.Tensor, sigma: torch.Tensor, l_grid: torch.Tensor, tol=1e-7) -> torch.Tensor:
    """
